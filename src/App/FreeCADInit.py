...
def setupSearchPaths(PathExtension):
    ...
    try:
        ...
        Log(
            "UnicodeDecodeError was raised when concatenating unicode string with PATH. Try to remove non-ascii paths...\n"
        )
        ...
        os.environ["PATH"] = (
            PathEnvironment.encode(errors="replace") + os.environ["PATH"]
        )
        Log("done\n")
    except KeyError:
...
def InitApplications():
    ...
    MacroDir = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Macro").GetString(
        "MacroPath"
    )
    ...
    # this path:
    additional_packages_path = os.path.join(
