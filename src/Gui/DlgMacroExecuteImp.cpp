...
void DlgMacroExecuteImp::onEditButtonClicked()
{
    ...
    QString file = QString::fromUtf8("%1/%2").arg(dir.absolutePath(), item->text(0));
    ...
    if (mitem->systemWide) {
        ...
        QString shownName;
        shownName = QString::fromUtf8("%1[*] - [%2]").arg(item->text(0), tr("Read-only"));
...
void DlgMacroExecuteImp::onCreateButtonClicked()
{
    ...
    if (!fn.isEmpty())
    {
        ...
        else
        {
            ...
            edit->setWindowTitle(QString::fromUtf8("%1[*]").arg(fn));
            edit->resize(400, 300);
            getMainWindow()->addWindow(edit);
            close();
        }
