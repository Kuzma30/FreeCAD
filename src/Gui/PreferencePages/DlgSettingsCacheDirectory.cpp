...
void DlgSettingsCacheDirectory::loadSettings()
{
    ...
    if (index < 0) {
        ui->comboBoxLimit->addItem(QLatin1String("%1 MB").arg(limit), limit);
...
{
    QString vendor = QLatin1String(App::Application::Config()["ExeVendor"].c_str());
...
bool ApplicationCache::performAction(qint64 total)
{
    ...
    if (total > limit) {
        ...
        msgBox.setText(QString::fromUtf8("%1 %2\n\n\n%3").arg(hint, ask, warn));
...
QString ApplicationCache::toString(qint64 size)
{
    ...
    return QString::fromUtf8("%1 %2").arg(QLocale().toString(outputSize, 'f', 2), units[i]);
}
