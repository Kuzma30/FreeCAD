...
class UpdateChecker:
    ...
    def check_package(self, package: Addon) -> None:
        ...
        if os.path.exists(clonedir):
            ...
            try:
                ...
                FreeCAD.Console.PrintWarning(
                    translate(
                        "AddonsInstaller", "Failed to read metadata from {name}"
    ...
    def check_macro(self, macro_wrapper: Addon) -> None:
        ...
        try:
            ...
            FreeCAD.Console.PrintWarning(
                translate(
                    "AddonsInstaller", "Failed to fetch code for macro '{name}'"
...
class FillMacroListWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        if not self.current_thread.isInterruptionRequested():
            self.status_message_signal.emit(
                translate("AddonsInstaller", "Retrieving macros from FreeCAD wiki")
            )
...
class CacheMacroCode(QtCore.QThread):
    ...
    def update_and_advance(self, repo: Addon) -> None:
        ...
        try:
            ...
            self.status_message.emit(
                translate("AddonsInstaller", "Getting metadata from macro {}").format(
                    next_repo.macro.name
                )
            )
            worker.start()
        except queue.Empty:
    ...
    def terminate(self, worker) -> None:
        if not worker.isFinished():
            ...
            FreeCAD.Console.PrintWarning(
                translate(
                    "AddonsInstaller", "Timeout while fetching metadata for macro {}"
                ).format(macro_name)
                + "\n"
            )
            worker.blockSignals(True)
            ...
            if worker.isRunning():
                FreeCAD.Console.PrintError(
                    translate(
                        "AddonsInstaller", "Failed to kill process for macro {}!\n"
                    ).format(macro_name)
                )
            with self.lock:
...
class InstallWorkbenchWorker(QtCore.QThread):
        ...
        with self.repo.git_lock:
            ...
            try:
                ...
                if self.repo.contains_workbench():
                    ...
                    answer = translate(
                        "AddonsInstaller", "Workbench successfully updated."
                    )
            except Exception as e:
                answer = (
    ...
    def run_git_clone(self, clonedir: str) -> None:
        ...
        if self.repo.contains_workbench():
            ...
            answer = translate("AddonsInstaller", "Addon successfully installed.")
    ...
    def update_zip_status(self, index: int, bytes_read: int, data_size: int):
        if index == self.zip_download_index:
            ...
            if data_size > 10 * 1024 * 1024:  # To avoid overflows, show MB instead
                ...
                self.status_message.emit(
                    translate(
                        "AddonsInstaller", "Downloading: {bytes_str}MB of unknown total"
    ...
    def finish_zip(self, index: int, response_code: int, filename: os.PathLike):
        ...
        if response_code != 200:
            self.failure.emit(
                self.repo,
                translate(
                    "AddonsInstaller", "Error: Error while downloading ZIP file for {}"
        ...
        self.success.emit(
            self.repo,
            translate(
                "AddonsInstaller", "Successfully installed {} from ZIP file"
            ).format(self.repo.display_name),
        )
    ...
    def run(self):
        ...
        for pymod in self.python_required:
            ...
            if proc.returncode != 0:
                self.failure.emit(
                    translate(
                        "AddonsInstaller", "Installation of Python package {} failed"
                    ).format(pymod),
                    proc.stderr,
                )
                return
...
class UpdateMetadataCacheWorker(QtCore.QThread):
    ...
    def process_requirements_txt(self, repo: Addon, data: QtCore.QByteArray):
        self.status_message.emit(
            translate("AddonsInstaller", "Downloaded requirements.txt for {}").format(
                repo.display_name
            )
        )
        f = io.StringIO(data.data().decode("utf8"))
