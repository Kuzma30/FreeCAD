...
class _ViewProviderAxis:
    ...
    def setProperties(self, vobj):
        ...
        ts = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetFloat(
            "textheight", 350
        )
        ...
        if not "DrawStyle" in pl:
            vobj.addProperty(
                ...
                "Axis",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The type of line to draw this axis"
                ),
            )
        ...
        if not "LabelOffset" in pl:
            vobj.addProperty(
                ...
                "Axis",
                QT_TRANSLATE_NOOP(
                    "App::Property", "A transformation to apply to each label"
                ),
            )
            ...
            if obj.Shape:
                if obj.Shape.Edges:
                    ...
                    for e in obj.Shape.Edges:
                        for v in e.Vertexes:
                            verts.append(
                                tuple(obj.Placement.inverse().multVec(v.Point))
                            )
                            vset.append(i)
                            i += 1
                        vset.append(-1)
                ...
                if vobj.Object.Shape:
                    if vobj.Object.Shape.Edges:
                        ...
                        for i in range(len(vobj.Object.Distances)):
                            for p in pos:
                                if (
                                    hasattr(vobj.Object, "Limit")
                                    and vobj.Object.Limit.Value
                                ):
                                    verts = [
                                        vobj.Object.Placement.inverse().multVec(
                                            vobj.Object.Shape.Edges[i].Vertexes[0].Point
                                        ),
                                        vobj.Object.Placement.inverse().multVec(
                                            vobj.Object.Shape.Edges[i + 1]
                                            .Vertexes[0]
                                            .Point
                                        ),
                                    ]
                                ...
                                if arrow == True:
                                    p3 = p2.add(
                                        Vector(chord).multiply(rad / 2).negative()
                                    )
                                    if vobj.BubblePosition.startswith("Arrow"):
                                        p4 = p3.add(
                                            Vector(dv).multiply(rad * 2).negative()
                                        )
                                        p5 = p2.add(
                                            Vector(dv).multiply(rad).negative()
                                        ).add(
                                    ...
                                    if vobj.BubblePosition.startswith("Arrow"):
                                        ...
                                        p5 = p2.add(
                                            Vector(dv).multiply(rad).negative()
                                        ).add(Vector(chord).multiply(rad * 1.5))
                                        pts = [
                                            tuple(p3),
                                            tuple(p4),
                                            tuple(p5),
                                        ...
                                        center = p5.add(
                                            Vector(chord).multiply(rad * 3).negative()
                                        )
                                    coords = coin.SoCoordinate3()
                                    coords.point.setValues(0, len(pts), pts)
                                    line = coin.SoFaceSet()
                                ...
                                self.bubbletexts.append(
                                    (tx, vobj.Object.Placement.multVec(center))
                                )
                                st.addChild(tr)
                                st.addChild(fo)
                                st.addChild(tx)
