        ...
        if Draft.getType(obj) in ["Floor", "Building"]:
            ...
            for parent in obj.InList:
                if hasattr(parent, "Group"):
                    ...
                    print(
                        "Warning: couldn't add new object '"
                        + label
                        + "' to parent object '"
                        + parent.Label
                        + "'"
...
class BuildingPart(ArchIFC.IfcProduct):
    ...
    def setProperties(self, obj):
        ...
        if not "HeightPropagate" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "If true, the height value propagates to contained objects",
                ),
            )
        ...
        if not "LevelOffset" in pl:
            obj.addProperty(
                ...
                "BuildingPart",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The level of the (0,0,0) point of this level"
                ),
            )
        if not "Area" in pl:
            obj.addProperty(
            ...
            if hasattr(self, "oldPlacement"):
                if self.oldPlacement and (self.oldPlacement != obj.Placement):
                    ...
                    for child in obj.Group:
                        if (
                            (not hasattr(child, "MoveWithHost")) or child.MoveWithHost
                        ) and hasattr(child, "Placement"):
                            ...
                            if deltar:
                                ...
                                print(
                                    ...
                                    shape.Placement.Rotation.Angle,
                                )
            ...
            if Draft.getType(child) in ["Wall", "Structure"]:
                if not child.Height.Value:
                    FreeCAD.Console.PrintLog(
                        "Auto-updating Height of " + child.Name + "\n"
                    )
    ...
    def autogroup(self, obj, child):
        ...
        if obj.ViewObject:
            if (
                hasattr(obj.ViewObject.Proxy, "autobbox")
                and obj.ViewObject.Proxy.autobbox
            ):
                if hasattr(child, "Shape") and child.Shape:
...
class ViewProviderBuildingPart:
    ...
    def setProperties(self, vobj):
        ...
        if not "OverrideUnit" in pl:
            vobj.addProperty(
                ...
                "BuildingPart",
                QT_TRANSLATE_NOOP(
                    "App::Property", "An optional unit to express levels"
                ),
            )
        if not "DisplayOffset" in pl:
            vobj.addProperty(
                ...
                "BuildingPart",
                QT_TRANSLATE_NOOP(
                    "App::Property", "A transformation to apply to the level mark"
                ),
            )
        ...
        if not "ShowUnit" in pl:
            vobj.addProperty(
                ...
                "BuildingPart",
                QT_TRANSLATE_NOOP(
                    "App::Property", "If true, show the unit on the level tag"
                ),
            )
        ...
        if not "ShowLabel" in pl:
            vobj.addProperty(
                ...
                "BuildingPart",
                QT_TRANSLATE_NOOP(
                    "App::Property", "If true, the object's label is displayed"
                ),
            )
            vobj.ShowLabel = True
        ...
        if not "ChildrenLineColor" in pl:
            ...
            c = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned(
                "DefaultShapeLineColor", 255
            )
            vobj.ChildrenLineColor = (
        ...
        if not "ChildrenShapeColor" in pl:
            vobj.addProperty(
                ...
                "ChildrenShapeColor",
                "Children",
                QT_TRANSLATE_NOOP("App::Property", "The shape color of child objects"),
            )
            c = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned(
        ...
        if not "AutoCutView" in pl:
            vobj.addProperty(
                ...
                "Clip",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Turn cutting on when activating this level"
                ),
            )
        ...
        if not "AutogroupAutosize" in pl:
            vobj.addProperty(
                ...
                "AutoGroup",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Automatically set size from contents"
                ),
            )
        if not "AutogroupMargin" in pl:
            vobj.addProperty(
                ...
                "AutoGroup",
                QT_TRANSLATE_NOOP(
                    "App::Property", "A margin to use when autosize is turned on"
                ),
            )
        ...
        if prop == "ShapeColor":
            ...
            if (
                ...
                if vobj.ShowLevel:
                    ...
                    try:
                        txt += units.display_external(
                            float(q), None, "Length", vobj.ShowUnit, u
                        )
                    except Exception:
            ...
            if (
                hasattr(vobj, "Visibility")
                and not (vobj.Visibility)
                and hasattr(vobj, "CutView")
            ):
                vobj.CutView = False
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        action1 = QtGui.QAction(
            QtGui.QIcon(":/icons/Draft_SelectPlane.svg"), "Set working plane", menu
        )
        QtCore.QObject.connect(
            action1, QtCore.SIGNAL("triggered()"), self.setWorkingPlane
        )
        menu.addAction(action1)
        action2 = QtGui.QAction(
            QtGui.QIcon(":/icons/Draft_SelectPlane.svg"), "Write camera position", menu
        )
