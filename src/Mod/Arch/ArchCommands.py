        ...
        if not hasattr(o, "Shape"):
            ...
            if (not s.isClosed()) and (not (Draft.getType(o) == "Axis")):
                ...
                for sol in s.Solids:
                    ...
                    if not sol.isClosed():
                        bad.append(
                            [o, translate("Arch", "contains a non-closed solid")]
                        )
                if len(s.Faces) != f:
                    bad.append(
                        [
                            o,
                            translate(
                                "Arch", "contains faces that are not part of any solid"
                            ),
                        ]
                    )
    return bad
...
def getHost(obj, strict=True):
    ...
    for par in obj.InList:
        if par.isDerivedFrom("Part::Feature") or par.isDerivedFrom(
        ...
        if obj.isDerivedFrom("Part::Feature"):
            if not (Draft.getType(obj) in ["Window", "Clone", "Pipe", "Rebar"]):
                for parent in obj.InList:
                    if parent.isDerivedFrom("Part::Feature") and not (
                        Draft.getType(parent)
                        in [
                            "Space",
                            "Facebinder",
                            "Window",
                        ...
                        if not parent.isDerivedFrom("Part::Part2DObject"):
                            ...
                            if hasattr(parent, "Host") and (parent.Host == obj):
                                ...
                                pass
                            elif hasattr(parent, "TypeId") and (
                                parent.TypeId == "Part::Mirroring"
                            ):
                                pass
                ...
                if newsels:
                    for o in newsels:
                        if hasattr(o.Object, "Shape"):
                            n = o.Object.Label
                            showUnit = FreeCAD.ParamGet(
                                "User parameter:BaseApp/Preferences/Mod/Arch"
                            ).GetBool("surveyUnits", True)
                            t = ""
...
class SurveyTaskPanel:
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            self.copyLength, QtCore.SIGNAL("clicked()"), self.clipLength
        )
    ...
    def retranslateUi(self, dlg):
        ...
        self.addButton.setText(
            QtGui.QApplication.translate("Arch", "Set description", None)
        )
        self.clearButton.setText(QtGui.QApplication.translate("Arch", "Clear", None))
        self.copyLength.setText(
            QtGui.QApplication.translate("Arch", "Copy Length", None)
        )
    ...
    def clipLength(self):
        if hasattr(FreeCAD, "SurveyObserver"):
            ...
            if FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetBool(
                ...
                QtGui.QApplication.clipboard().setText(
                    str(u.Value / u.getUserPreferred()[1])
                )
...
def toggleIfcBrepFlag(obj):
    ...
    if not hasattr(obj, "IfcAttributes"):
        FreeCAD.Console.PrintMessage(
            translate("Arch", "Object doesn't have settable IFC Attributes")
        )
    else:
        d = obj.IfcAttributes
        if "FlagForceBrep" in d.keys():
            if d["FlagForceBrep"] == "True":
                ...
                FreeCAD.Console.PrintMessage(
                    translate("Arch", "Disabling Brep force flag of object")
                    + " "
                    + obj.Label
                    + "\n"
                )
                ...
                FreeCAD.Console.PrintMessage(
                    translate("Arch", "Enabling Brep force flag of object")
                    + " "
                    + obj.Label
                    + "\n"
                )
        ...
        if ok:
            ...
            if faces[pair[0]][0].CenterOfMass.z < faces[pair[1]][0].CenterOfMass.z:
                valids.append(
                    [
                        faces[pair[0]][0],
                        faces[pair[1]][0].CenterOfMass.sub(
                            faces[pair[0]][0].CenterOfMass
                        ),
                    ]
                )
            else:
                valids.append(
                    [
                        faces[pair[1]][0],
                        faces[pair[0]][0].CenterOfMass.sub(
                            faces[pair[1]][0].CenterOfMass
                        ),
                    ]
                )
    if valids:
    ...
    def Activated(self):
        ...
        if Draft.getType(sel[-1]) == "Space":
            FreeCAD.ActiveDocument.openTransaction(
                translate("Arch", "Add space boundary")
            )
...
class _CommandRemove:
    ...
    def Activated(self):
        ...
        if Draft.getType(sel[-1]) == "Space":
            FreeCAD.ActiveDocument.openTransaction(
                translate("Arch", "Remove space boundary")
            )
            FreeCADGui.addModule("Arch")
            FreeCADGui.doCommand(
...
class _CommandRemoveShape:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Arch_RemoveShape", "Remove Shape from Arch"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
...
class _CommandCheck:
    ...
    def Activated(self):
        ...
        if not result:
            FreeCAD.Console.PrintMessage(
                str(translate("Arch", "All good! No problems found"))
            )
        else:
            FreeCADGui.Selection.clearSelection()
            for i in result:
                FreeCAD.Console.PrintWarning(
                    "Object " + i[0].Name + " (" + i[0].Label + ") " + utf8_decode(i[1])
                )
...
class _CommandComponent:
    ...
    def Activated(self):
        ...
        if sel:
            FreeCAD.ActiveDocument.openTransaction(
                translate("Arch", "Create Component")
            )
            FreeCADGui.addModule("Arch")
...
class _CommandCloneComponent:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Arch_CloneComponent",
                "Clones an object as an undefined architectural component",
            ),
        }
...
def makeIfcSpreadsheet(archobj=None):
    ...
    ifc_spreadsheet = FreeCAD.ActiveDocument.addObject(
        "Spreadsheet::Sheet", "IfcProperties"
    )
    ...
    if archobj:
        if hasattr(obj, "IfcProperties"):
            ...
            FreeCAD.Console.PrintWarning(
                translate(
                    "Arch",
                    "The object doesn't have an IfcProperties attribute. Cancel spreadsheet creation for object:",
                )
                + " "
                + archobj.Label
            )
