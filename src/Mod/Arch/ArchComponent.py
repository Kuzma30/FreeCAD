                ...
                if mod == "Base":
                    ...
                    if mod != "Objects":
                        ...
                        if Draft.getType(compobject) == "PanelSheet":
                            addobject.Placement.move(
                                compobject.Placement.Base.negative()
                            )
        else:
            for a in attribs[:3]:
                if hasattr(compobject, a):
...
class Component(ArchIFC.IfcProduct):
    ...
    def setProperties(self, obj):
        ...
        if not "Base" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The base object this component is built upon"
                ),
            )
        if not "CloneOf" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The object this component is cloning"
                ),
            )
        if not "Additions" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Other shapes that are appended to this object"
                ),
            )
        ...
        if not "Description" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "An optional description for this component"
                ),
            )
        if not "Tag" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "An optional tag for this component"
                ),
            )
        ...
        if not "MoveBase" in pl:
            ...
            obj.MoveBase = FreeCAD.ParamGet(
                "User parameter:BaseApp/Preferences/Mod/Arch"
            ).GetBool("MoveBase", False)
        if not "MoveWithHost" in pl:
            ...
            obj.MoveWithHost = FreeCAD.ParamGet(
                "User parameter:BaseApp/Preferences/Mod/Arch"
            ).GetBool("MoveWithHost", False)
        if not "VerticalArea" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The area of all vertical faces of this object"
                ),
            )
        ...
        if not "PerimeterLength" in pl:
            obj.addProperty(
                ...
                "Component",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The perimeter length of the horizontal area"
                ),
            )
                ...
                if self.oldPlacement:
                    ...
                    for child in self.getMovableChildren(obj):
                        ...
                        if deltar:
                            ...
                            shape.rotate(
                                DraftVecUtils.tup(self.oldPlacement.Base),
                ...
                if obj.Base.Base:
                    if obj.Placement.Rotation.Angle < 0.0001:
                        ...
                        if data:
                            ...
                            disp = obj.Shape.CenterOfMass.sub(
                                obj.Base.Shape.CenterOfMass
                            )
            ...
            if base:
                ...
                if subvolume:
                    if base.Solids and subvolume.Solids:
                        if placement:
                            subvolume.Placement = subvolume.Placement.multiply(
                                placement
                            )
                        if len(base.Solids) > 1:
                            base = Part.makeCompound(
                                [sol.cut(subvolume) for sol in base.Solids]
                            )
                        else:
                ...
                if shape.isValid():
                    if shape.Solids:
                        ...
                        if shape.Volume < 0:
                            FreeCAD.Console.PrintError(
                                translate(
                                    "Arch", "Error computing the shape of this object"
                                )
                                + "\n"
                            )
                            return
                    ...
                    if allowinvalid:
                        ...
                        FreeCAD.Console.PrintWarning(
                            obj.Label
                            + " "
                            + translate("Arch", "has an invalid shape")
                            + "\n"
                        )
            else:
                FreeCAD.Console.PrintWarning(
    ...
    def computeAreas(self, obj):
        ...
        fmax = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetInt(
            "MaxComputeAreas", 20
        )
        ...
        for i, f in enumerate(obj.Shape.Faces):
            try:
                ...
                print(
                    "Debug: Error computing areas for ",
                    obj.Label,
                    ": normalAt() Face ",
                    i,
                )
            ...
            for f in fset:
                if f.normalAt(0, 0).getAngle(FreeCAD.Vector(0, 0, 1)) < 0.00001:
                    ...
                    try:
                        pf = Part.Face(
                            Part.Wire(
                                TechDraw.project(f, FreeCAD.Vector(0, 0, 1))[0].Edges
                            )
                        )
                    except Part.OCCError:
...
class ViewProviderComponent:
    ...
    def setProperties(self, vobj):
        ...
        if not "UseMaterialColor" in vobj.PropertiesList:
            ...
            vobj.UseMaterialColor = FreeCAD.ParamGet(
                "User parameter:BaseApp/Preferences/Mod/Arch"
            ).GetBool("UseMaterialColor", True)
    ...
    def updateData(self, obj, prop):
        ...
        if prop == "Material":
            if obj.Material and (
                (not hasattr(obj.ViewObject, "UseMaterialColor"))
                or obj.ViewObject.UseMaterialColor
            ):
            ...
            if hasattr(vobj.Object, "CloneOf"):
                if vobj.Object.CloneOf and hasattr(vobj.Object.CloneOf, "DiffuseColor"):
                    if len(vobj.Object.CloneOf.ViewObject.DiffuseColor) > 1:
                        if (
                            vobj.DiffuseColor
                            != vobj.Object.CloneOf.ViewObject.DiffuseColor
                        ):
                            vobj.DiffuseColor = (
                                vobj.Object.CloneOf.ViewObject.DiffuseColor
                            )
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        QtCore.QObject.connect(
            action1, QtCore.SIGNAL("triggered()"), self.toggleSubcomponents
        )
...
class ComponentTaskPanel:
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            self.ifcButton, QtCore.SIGNAL("clicked()"), self.editIfcProperties
        )
        QtCore.QObject.connect(
            self.classButton, QtCore.SIGNAL("clicked()"), self.editClass
        )
        QtCore.QObject.connect(
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.baseform.setWindowTitle(
            QtGui.QApplication.translate("Arch", "Component", None)
        )
        self.delButton.setText(QtGui.QApplication.translate("Arch", "Remove", None))
        self.addButton.setText(QtGui.QApplication.translate("Arch", "Add", None))
        self.title.setText(
            QtGui.QApplication.translate("Arch", "Components of this object", None)
        )
        self.treeBase.setText(
            0, QtGui.QApplication.translate("Arch", "Base component", None)
        )
        self.treeAdditions.setText(
            0, QtGui.QApplication.translate("Arch", "Additions", None)
        )
        self.treeSubtractions.setText(
            0, QtGui.QApplication.translate("Arch", "Subtractions", None)
        )
        self.treeObjects.setText(
            0, QtGui.QApplication.translate("Arch", "Objects", None)
        )
        self.treeAxes.setText(0, QtGui.QApplication.translate("Arch", "Axes", None))
        self.treeComponents.setText(
            0, QtGui.QApplication.translate("Arch", "Components", None)
        )
        self.treeFixtures.setText(
            0, QtGui.QApplication.translate("Arch", "Fixtures", None)
        )
        ...
        self.ifcButton.setText(
            QtGui.QApplication.translate("Arch", "Edit IFC properties", None)
        )
        self.classButton.setText(
            QtGui.QApplication.translate("Arch", "Edit standard code", None)
        )
    ...
    def editIfcProperties(self):
        ...
        self.ifcEditor.comboProperty.addItems(
            [QtGui.QApplication.translate("Arch", "Add property...", None)]
            + self.plabels
        )
        ...
        # Add callbacks
        QtCore.QObject.connect(
            self.ifcEditor.buttonBox,
            QtCore.SIGNAL("accepted()"),
            self.acceptIfcProperties,
        )
        QtCore.QObject.connect(
        ...
        QtCore.QObject.connect(
            self.ifcEditor.comboPset,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.addIfcPset,
        )
        QtCore.QObject.connect(
            self.ifcEditor.buttonDelete,
            QtCore.SIGNAL("clicked()"),
            self.removeIfcProperty,
        )
        self.ifcEditor.treeProperties.setSortingEnabled(True)

        # set checkboxes
        if "FlagForceBrep" in self.obj.IfcData:
            self.ifcEditor.checkBrep.setChecked(
                self.obj.IfcData["FlagForceBrep"] == "True"
            )
        if "FlagParametric" in self.obj.IfcData:
            self.ifcEditor.checkParametric.setChecked(
                self.obj.IfcData["FlagParametric"] == "True"
            )
        self.ifcEditor.show()
    ...
    def addIfcPset(self, idx=0):
        ...
        if hasattr(self, "ifcEditor") and self.ifcEditor:
            ...
            if idx != 0:
                ...
                for i in range(self.ifcModel.rowCount()):
                    if self.ifcModel.item(i, 0).hasChildren():
                        self.ifcEditor.treeProperties.setFirstColumnSpanned(
                            i, idx, True
                        )
                self.ifcEditor.treeProperties.expandAll()
