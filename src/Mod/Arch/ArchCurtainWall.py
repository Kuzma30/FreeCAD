    ...
    def Activated(self):
        ...
        if len(sel) > 1:
            ...
            FreeCAD.ActiveDocument.openTransaction(
                translate("Arch", "Create Curtain Wall")
            )
...
class CurtainWall(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Host" in pl:
            obj.addProperty(
                ...
                "CurtainWall",
                QT_TRANSLATE_NOOP(
                    "App::Property", "An optional host object for this curtain wall"
                ),
            )
        if not "Height" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The height of the curtain wall, if based on an edge",
                ),
            )
        ...
        if not "SwapHorizontalVertical" in pl:
            obj.addProperty(
                ...
                "CurtainWall",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Swaps horizontal and vertical lines"
                ),
            )
        if not "Refine" in pl:
            obj.addProperty(
        ...
        if not "CenterProfiles" in pl:
            obj.addProperty(
                ...
                "CurtainWall",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Centers the profile over the edges or not"
                ),
            )
        ...
        if dedges:
            ...
            if dprofile:
                for dedge in dedges:
                    ...
                    dmullions.append(
                        self.makeMullion(dedge, dprofile, rot, obj.CenterProfiles)
                    )
    ...
    def makeMullion(self, edge, profile, rotation, recenter=False):
        ...
        if rotation:
            mullion = mullion.rotate(
                center, rotation.Axis, math.degrees(rotation.Angle)
            )
    ...
    def getMullionProfile(self, obj, direction):
        ...
        if prof:
            ...
            if (not proh) or (not prow):
                return None
            profile = Part.Face(
                Part.makePlane(prow, proh, FreeCAD.Vector(-prow / 2, -proh / 2, 0))
            )
        return profile
...
class ViewProviderCurtainWall(ArchComponent.ViewProviderComponent):
    ...
    def colorize(self, obj, force=False):
            ...
            if obj.Material.Names:
                ...
                if "Glass panel" in obj.Material.Names:
                    mat = obj.Material.Materials[
                        obj.Material.Names.index("Glass panel")
                    ]
                    if ("DiffuseColor" in mat.Material) and (
                        "(" in mat.Material["DiffuseColor"]
                    ):
                    ...
                    mat = obj.Material.Materials[
                        obj.Material.Names.index("Solid panel")
                    ]
                    if ("DiffuseColor" in mat.Material) and (
                        "(" in mat.Material["DiffuseColor"]
                    ):
        ...
        colors = []
        nmullions = (
            obj.VerticalMullionNumber
            + obj.HorizontalMullionNumber
            + obj.DiagonalMullionNumber
        )
