    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Arch_CutLine", "Cut an object with a line"
            ),
        }
    ...
    def Activated(self):
        ...
        if len(sel) != 2:
            FreeCAD.Console.PrintError(
                "You must select exactly two objects, the shape to be cut and a line\n"
            )
            return
        if not sel[1].SubObjects:
            FreeCAD.Console.PrintError(
                "You must select a line from the second object (cut line), not the whole object\n"
            )
            return
...
class _CommandCutPlane:
    ...
    def Activated(self):
        ...
        if len(sel) != 2:
            FreeCAD.Console.PrintError(
                "You must select exactly two objects, the shape to be cut and the cut plane\n"
            )
            return
        if not sel[1].SubObjects:
            FreeCAD.Console.PrintError(
...
class _CutPlaneTaskPanel:
    def __init__(self, linecut=False):
        ...
        self.plan = None
        if linecut:
            self.plan = getPlanWithLine(
                FreeCADGui.Selection.getSelectionEx()[1].SubObjects[0]
            )
        else:
        ...
        QtCore.QObject.connect(
            ...
            self.previewCutVolume,
        )
        self.previewObj = FreeCAD.ActiveDocument.addObject(
        ...
        if len(s) > 1:
            if s[1].SubObjects:
                ...
                cutComponentwithPlane(
                    FreeCADGui.Selection.getSelectionEx()[0], self.plan, val
                )
    ...
    def previewCutVolume(self, i):
        cutVolume = ArchCommands.getCutVolume(
            self.plan, FreeCADGui.Selection.getSelectionEx()[0].Object.Shape
        )
        FreeCAD.ActiveDocument.removeObject(self.previewObj.Name)
        self.previewObj = FreeCAD.ActiveDocument.addObject(
            "Part::Feature", "PreviewCutVolume"
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.infoText.setText(
            QtGui.QApplication.translate("Arch", "Which side to cut", None)
        )
