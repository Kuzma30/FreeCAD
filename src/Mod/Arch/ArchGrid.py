    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_Grid", "Creates a customizable grid object"
            ),
        }
    ...
    def setProperties(self, obj):
        ...
        if not "RowSize" in pl:
            obj.addProperty(
                ...
                "RowSize",
                "Grid",
                QT_TRANSLATE_NOOP("Arch_Grid", "The sizes for rows"),
            )
        ...
        if not "Spans" in pl:
            obj.addProperty(
                ...
                "Grid",
                QT_TRANSLATE_NOOP(
                    "Arch_Grid", "The span ranges of cells that are merged together"
                ),
            )
        ...
        if not "AutoWidth" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "Arch_Grid",
                    "Creates automatic column divisions (set to 0 to disable)",
                ),
            )
    ...
    def getSizes(self, obj):
        ...
        if obj.AutoHeight.Value:
            if obj.AutoHeight.Value > obj.Height.Value:
                FreeCAD.Console.PrintError(
                    translate("Arch", "Auto height is larger than height")
                )
                return [], [], []
        ...
        if obj.AutoWidth.Value:
            if obj.AutoWidth.Value > obj.Width.Value:
                FreeCAD.Console.PrintError(
                    translate("Arch", "Auto width is larger than width")
                )
                return [], [], []
...
class ArchGridTaskPanel:
    ...
    def __init__(self, obj):
        ...
        QtCore.QObject.connect(
            self.widthUi, QtCore.SIGNAL("valueChanged(double)"), self.setWidth
        )
        QtCore.QObject.connect(
            self.heightUi, QtCore.SIGNAL("valueChanged(double)"), self.setHeight
        )
        QtCore.QObject.connect(
            self.table, QtCore.SIGNAL("itemSelectionChanged()"), self.checkSpan
        )
        QtCore.QObject.connect(
            self.addRowButton, QtCore.SIGNAL("clicked()"), self.addRow
        )
        QtCore.QObject.connect(
            self.delRowButton, QtCore.SIGNAL("clicked()"), self.delRow
        )
        QtCore.QObject.connect(
            self.addColumnButton, QtCore.SIGNAL("clicked()"), self.addColumn
        )
        QtCore.QObject.connect(
            self.delColumnButton, QtCore.SIGNAL("clicked()"), self.delColumn
        )
        QtCore.QObject.connect(
            self.spanButton, QtCore.SIGNAL("clicked()"), self.addSpan
        )
        QtCore.QObject.connect(
            self.delSpanButton, QtCore.SIGNAL("clicked()"), self.removeSpan
        )
        QtCore.QObject.connect(
    ...
    def retranslateUi(self, widget=None):
        ...
        self.addColumnButton.setText(
            QtGui.QApplication.translate("Arch", "Add col", None)
        )
        self.delColumnButton.setText(
            QtGui.QApplication.translate("Arch", "Del col", None)
        )
        self.spanButton.setText(
            QtGui.QApplication.translate("Arch", "Create span", None)
        )
        self.delSpanButton.setText(
            QtGui.QApplication.translate("Arch", "Remove span", None)
        )
        self.title.setText(
    ...
    def update(self):
        ...
        if self.obj.Rows:
            ...
            for i, v in enumerate(self.obj.RowSize):
                if i < len(vlabels):
                    vlabels[i] = FreeCAD.Units.Quantity(
                        v, FreeCAD.Units.Length
                    ).getUserPreferred()[0]
        ...
        if self.obj.Columns:
            ...
            for i, v in enumerate(self.obj.ColumnSize):
                if i < len(hlabels):
                    hlabels[i] = FreeCAD.Units.Quantity(
                        v, FreeCAD.Units.Length
                    ).getUserPreferred()[0]
    ...
    def addSpan(self):
        ...
        for r in self.table.selectedRanges():
            if r.rowCount() * r.columnCount() > 1:
                self.table.setSpan(
                    r.topRow(), r.leftColumn(), r.rowCount(), r.columnCount()
                )
                self.spans.append(
                    [r.topRow(), r.leftColumn(), r.rowCount(), r.columnCount()]
                )
