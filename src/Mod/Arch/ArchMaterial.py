...
class _CommandArchMultiMaterial:
    ...
    def GetResources(self):
        ...
        return {
            ...
            "Accel": "M, T",
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_MultiMaterial", "Creates or edits multi-materials"
            ),
        }
    ...
    def Activated(self):
        ...
        FreeCAD.ActiveDocument.openTransaction(
            translate("Arch", "Create multi-material")
        )
        FreeCADGui.addModule("Arch")
        FreeCADGui.Control.closeDialog()
        ...
        for obj in sel:
            if hasattr(obj, "Material"):
                if not obj.isDerivedFrom("App::MaterialObject"):
                    FreeCADGui.doCommand(
                        "FreeCAD.ActiveDocument." + obj.Name + ".Material = mat"
                    )
        FreeCADGui.doCommandGui("mat.ViewObject.Document.setEdit(mat.ViewObject, 0)")
        FreeCAD.ActiveDocument.commitTransaction()
        FreeCAD.ActiveDocument.recompute()
...
class _ViewProviderArchMaterialContainer:
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        action1 = QtGui.QAction(
            QtGui.QIcon(":/icons/Arch_Material_Group.svg"), "Merge duplicates", menu
        )
    ...
    def mergeByName(self):
        if hasattr(self, "Object"):
            mats = [
                o for o in self.Object.Group if o.isDerivedFrom("App::MaterialObject")
            ]
            todelete = []
            for mat in mats:
                orig = None
            ...
            for tod in todelete:
                if not tod.InList:
                    FreeCAD.Console.PrintMessage(
                        "Merging duplicate material " + tod.Label + "\n"
                    )
    ...
    def setProperties(self, obj):
        ...
        if not "StandardCode" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Material",
                QT_TRANSLATE_NOOP(
                    "App::Property", "A standard code (MasterFormat, OmniClass,...)"
                ),
            )
        ...
        if not "Transparency" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Material",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The transparency value of this material"
                ),
            )
        ...
        if not "SectionColor" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Material",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The color of this material when cut"
                ),
            )
        ...
        if prop == "Material":
            if "SectionColor" in obj.Material:
                c = tuple(
                    [
                        float(f)
                        for f in obj.Material["SectionColor"]
                        .strip("()")
                        .strip("[]")
                        .split(",")
                    ]
            ...
            if "DiffuseColor" in obj.Material:
                c = tuple(
                    [
                        float(f)
                        for f in obj.Material["DiffuseColor"]
                        .strip("()")
                        .strip("[]")
                        .split(",")
                    ]
        ...
        if obj.Material:
            if FreeCAD.GuiUp:
                if "DiffuseColor" in obj.Material:
                    ...
                    for p in obj.InList:
                        if hasattr(p, "Material") and (
                            (not hasattr(p.ViewObject, "UseMaterialColor"))
                            or p.ViewObject.UseMaterialColor
                        ):
...
class _ViewProviderArchMaterial:
    ...
    def updateData(self, obj, prop):
        if prop == "Color":
            ...
            if hasattr(obj, "Color"):
                c = obj.Color
                matcolor = QtGui.QColor(
                    int(c[0] * 255), int(c[1] * 255), int(c[2] * 255)
                )
                darkcolor = QtGui.QColor(
                    int(c[0] * 125), int(c[1] * 125), int(c[2] * 125)
                )
                ...
                pt.setPen(
                    QtGui.QPen(
                        QtCore.Qt.black, 2, QtCore.Qt.SolidLine, QtCore.Qt.FlatCap
                    )
                )
                ...
                pt.setPen(
                    QtGui.QPen(
                        QtCore.Qt.white, 1, QtCore.Qt.SolidLine, QtCore.Qt.FlatCap
                    )
                )
...
class _ArchMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        QtCore.QObject.connect(
            self.form.comboBox_MaterialsInDir,
            QtCore.SIGNAL("currentIndexChanged(QString)"),
            self.chooseMat,
        )
        QtCore.QObject.connect(
            self.form.comboBox_FromExisting,
        ...
        QtCore.QObject.connect(
            self.form.ButtonColor, QtCore.SIGNAL("pressed()"), self.getColor
        )
        QtCore.QObject.connect(
            self.form.ButtonSectionColor,
            QtCore.SIGNAL("pressed()"),
            self.getSectionColor,
        )
        QtCore.QObject.connect(
            self.form.ButtonUrl, QtCore.SIGNAL("pressed()"), self.openUrl
        )
        QtCore.QObject.connect(
            self.form.ButtonEditor, QtCore.SIGNAL("pressed()"), self.openEditor
        )
        QtCore.QObject.connect(
            self.form.ButtonCode, QtCore.SIGNAL("pressed()"), self.getCode
        )
    ...
    def setFields(self):
        ...
        if "DiffuseColor" in self.material:
            self.form.ButtonColor.setIcon(
                self.getColorIcon(self.material["DiffuseColor"])
            )
        ...
        if "SectionColor" in self.material:
            self.form.ButtonSectionColor.setIcon(
                self.getColorIcon(self.material["SectionColor"])
            )
        ...
        for o in FreeCAD.ActiveDocument.Objects:
            if o.isDerivedFrom("App::MaterialObject"):
                if o != self.obj:
                    ...
                    if o.Label == father:
                        self.form.comboFather.setCurrentIndex(
                            self.form.comboFather.count() - 1
                        )
                        found = True
    ...
    def getFields(self):
        ...
        self.material["DiffuseColor"] = self.getColorFromIcon(
            self.form.ButtonColor.icon()
        )
        self.material["ViewColor"] = self.material["DiffuseColor"]
        self.material["Color"] = self.material["DiffuseColor"]
        self.material["SectionColor"] = self.getColorFromIcon(
            self.form.ButtonSectionColor.icon()
        )
...
class _ArchMultiMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        QtCore.QObject.connect(
            self.form.chooseCombo,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.fromExisting,
        )
        QtCore.QObject.connect(
            self.form.addButton, QtCore.SIGNAL("pressed()"), self.addLayer
        )
        QtCore.QObject.connect(
            self.form.upButton, QtCore.SIGNAL("pressed()"), self.upLayer
        )
        QtCore.QObject.connect(
            self.form.downButton, QtCore.SIGNAL("pressed()"), self.downLayer
        )
        QtCore.QObject.connect(
            self.form.delButton, QtCore.SIGNAL("pressed()"), self.delLayer
        )
        QtCore.QObject.connect(
            self.form.invertButton, QtCore.SIGNAL("pressed()"), self.invertLayer
        )
        QtCore.QObject.connect(
            self.model,
    ...
    def invertLayer(self):
        items = [
            self.model.takeRow(row) for row in range(self.model.rowCount() - 1, -1, -1)
        ]
        items.reverse()
        for item in items:
