...
class Nester:
    def __init__(self, container=None, shapes=None):
        ...
        self.progress = 0
        self.setCounter = (
            None  # optionally define a setCounter(value) function where value is a %
        )
            ...
            for edge in face[1].OuterWire.OrderedEdges:
                if isinstance(edge.Curve, (Part.LineSegment, Part.Line)):
                    ...
                    for i in range(DISCRETIZE):
                        ...
                        par = (
                            edge.FirstParameter
                            + (edge.LastParameter - edge.FirstParameter) * s
                        )
        ...
        while faces:
            ...
            print(
                ...
                FreeCAD.Units.Quantity(
                    faces[-1][1].Area, FreeCAD.Units.Area
                ).getUserPreferred()[0],
                ": ",
                end="",
            )
            ...
            for rotation in ROTATIONS:
                ...
                for sheetnumber, sheet in enumerate(sheets):
                                            ...
                                            if True:
                                                ...
                                                if p:
                                                    ...
                                                    if (
                                                        ...
                                                        and (
                                                            p.sub(p2).Length > TOLERANCE
                                                        )
                                                        and (
                                                            p.sub(p3).Length > TOLERANCE
                                                        )
                                                        and (
                                                            p.sub(p4).Length > TOLERANCE
                                                        )
                                                    ):
                                                        outside = False
                                                        break
                    ...
                    if (fitpol.Area > 0) and fitpol.Vertexes:
                        ...
                        fitverts = sorted(
                            [v.Point for v in fitpol.Vertexes], key=lambda v: v.x
                        )
                        for p in fitverts:
                            if not self.update():
                                return
            ...
            if available:
                ...
                verts = sorted(
                    [v.Point for v in initials[0][0].Vertexes], key=lambda v: v.x
                )
    ...
    def show(self, result=None):
        ...
        for sheet in result:
            ...
            feats.append(o)
            offset = offset.add(
                FreeCAD.Vector(1.1 * self.container.BoundBox.XLength, 0, 0)
            )
    ...
    def getPlacements(self, result=None):
        ...
        for sheet in result:
            for face in sheet:
                ...
                if not orig:
                    print(
                        "error: hashCode mismatch between original and transformed face"
                    )
                    return
                shape = face[1]
                if offset.Length:
