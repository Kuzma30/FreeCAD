                ...
                if hasattr(obj, "Shape"):
                    if len(obj.Shape.Wires) == 1:
                        FreeCAD.ActiveDocument.openTransaction(
                            translate("Arch", "Create Pipe")
                        )
                        FreeCADGui.addModule("Arch")
                        FreeCADGui.doCommand(
                            "obj = Arch.makePipe(FreeCAD.ActiveDocument."
                            + obj.Name
                            + ")"
                        )
...
class _CommandPipeConnector:
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_PipeConnector",
                "Creates a connector between 2 or 3 selected pipes",
            ),
        }
...
class _ArchPipe(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Diameter" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The diameter of this pipe, if not based on a profile",
                ),
            )
        if not "Length" in pl:
            obj.addProperty(
        ...
        if not "WallThickness" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The wall thickness of this pipe, if not based on a profile",
                ),
            )
        self.Type = "Pipe"
    ...
    def execute(self, obj):
        ...
        if obj.OffsetStart.Value:
            ...
            e = Part.LineSegment(
                e.Vertexes[0].Point.add(v), e.Vertexes[-1].Point
            ).toShape()
        ...
        if obj.OffsetEnd.Value:
            ...
            e = Part.LineSegment(
                e.Vertexes[-1].Point.add(v), e.Vertexes[0].Point
            ).toShape()
    ...
    def getWire(self, obj):
        ...
        if obj.Base:
            ...
            if obj.Base.Shape.Wires[0].isClosed():
                FreeCAD.Console.PrintError(
                    translate("Arch", "The base wire is closed") + "\n"
                )
                return
    ...
    def getProfile(self, obj):
        ...
        if obj.Profile:
            if not obj.Profile.getLinkedObject().isDerivedFrom("Part::Part2DObject"):
                FreeCAD.Console.PrintError(
                    translate("Arch", "The profile is not a 2D Part") + "\n"
                )
                return
            if not obj.Profile.Shape.Wires[0].isClosed():
                FreeCAD.Console.PrintError(
                    translate("Arch", "The profile is not closed") + "\n"
                )
                return
            ...
            if obj.WallThickness.Value and (
                obj.WallThickness.Value < obj.Diameter.Value / 2
            ):
                p2 = Part.Wire(
                    [
                        Part.Circle(
...
class _ArchPipeConnector(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Radius" in pl:
            obj.addProperty(
                ...
                "PipeConnector",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The curvature radius of this connector"
                ),
            )
        if not "Pipes" in pl:
            obj.addProperty(
                ...
                "PipeConnector",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The pipes linked by this connector"
                ),
            )
    ...
    def execute(self, obj):
        ...
        if wires[0].Vertexes[0].Point.sub(wires[1].Vertexes[0].Point).Length <= ptol:
            ...
            wires[0].Vertexes[-1].Point.sub(wires[1].Vertexes[-1].Point).Length <= ptol
        ):
            order = ["end", "end"]
        ...
        if order[0] == "start":
            ...
            v1 = (
                wires[0].Vertexes[-2].Point.sub(wires[0].Vertexes[-1].Point).normalize()
            )
        if order[1] == "start":
            v2 = wires[1].Vertexes[1].Point.sub(wires[1].Vertexes[0].Point).normalize()
        else:
            v2 = (
                wires[1].Vertexes[-2].Point.sub(wires[1].Vertexes[-1].Point).normalize()
            )
        ...
        if len(obj.Pipes) == 2:
            ...
            if order[2] == "start":
                v3 = (
                    wires[2]
                    .Vertexes[1]
                    .Point.sub(wires[2].Vertexes[0].Point)
                    .normalize()
                )
            else:
                v3 = (
                    wires[2]
                    .Vertexes[-2]
                    .Point.sub(wires[2].Vertexes[-1].Point)
                    .normalize()
                )
            if round(v1.getAngle(v2), tol) in [0, round(math.pi, tol)]:
                pair = [v1, v2, v3]
