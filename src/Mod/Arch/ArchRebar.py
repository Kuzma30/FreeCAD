...
def makeRebar(
    baseobj=None, sketch=None, diameter=None, amount=1, offset=None, name="Rebar"
):
...
class _CommandRebar:
    ...
    def Activated(self):
                    ...
                    if hasattr(obj, "Support"):
                        if obj.Support:
                            if len(obj.Support) != 0:
                                support = (
                                    "FreeCAD.ActiveDocument." + obj.Support[0][0].Name
                                )
                    FreeCAD.ActiveDocument.openTransaction(
                        translate("Arch", "Create Rebar")
                    )
        ...
        FreeCAD.Console.PrintMessage(
            translate("Arch", "Please select a base face on a structural object") + "\n"
        )
...
class _Rebar(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Host" in pl:
            obj.addProperty(
                ...
                "Rebar",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The structure object that hosts this rebar"
                ),
            )
    ...
    def execute(self, obj):
        ...
        if not obj.Base:
            ...
            return
        if (
            not hasattr(obj.Base, "Shape")
            or (not obj.Base.Shape)
            or obj.Base.Shape.isNull()
        ):
            FreeCAD.Console.PrintError(
        ...
        if hasattr(obj, "Direction"):
            if not DraftVecUtils.isNull(obj.Direction):
                ...
                if fathershape:
                    size = (
                        ArchCommands.projectToVector(fathershape.copy(), axis)
                    ).Length
                else:
                    size = 1
        if hasattr(obj, "Distance"):
        ...
        if hasattr(obj, "CustomSpacing"):
            if obj.CustomSpacing:
                ...
                influenceArea = (
                    sum(spacinglist) - spacinglist[0] / 2 - spacinglist[-1] / 2
                )
        ...
        if prop == "Shape":
            ...
            if hasattr(obj.Proxy, "wires"):
                if obj.Proxy.wires:
                    ...
                    pts = re.findall(
                        "point \[(.*?)\]", comp.writeInventor().replace("\n", "")
                    )
...
def CustomSpacingPlacement(
    spacinglist, barnumber, axis, rotation, offsetstart, offsetend
):
