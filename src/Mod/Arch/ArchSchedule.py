...
class _ArchSchedule:
    ...
    def setProperties(self, obj):
        ...
        if not "Description" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Description",
                "Arch",
                QT_TRANSLATE_NOOP("App::Property", "The description column"),
            )
        ...
        if not "Result" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Arch",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The spreadsheet to print the results to"
                ),
            )
        ...
        if prop == "CreateSpreadsheet":
            if hasattr(obj, "CreateSpreadsheet") and obj.CreateSpreadsheet:
                if not obj.Result:
                    ...
                    sp = FreeCAD.ActiveDocument.addObject(
    ...
    def execute(self, obj):
        ...
        self.data = (
            {}
        )  # store all results in self.data, so it lives even without spreadsheet
            ...
            if val:
                ...
                if val.upper() == "COUNT":
                    ...
                    for o in objs:
                        ...
                        try:
                            ...
                            FreeCAD.Console.PrintWarning(
                                translate(
                                    "Arch", "Unable to retrieve value from object"
                                )
                                + ": "
                                + o.Name
                                + "."
                            ...
                            if obj.DetailedResults:
                                ...
                                if tp and unit:
                                    ...
                                    self.data["B" + str(li)] = str(
                                        q.getValueAs(unit).Value
                                    )
                    ...
                    if verbose:
                        if tp and unit:
                            v = fs.format(
                                FreeCAD.Units.Quantity(val, tp).getValueAs(unit).Value
                            )
...
class _ViewProviderArchSchedule:
    ...
    def setupContextMenu(self, vobj, menu):
        action1 = QtGui.QAction(
            QtGui.QIcon(":/icons/Arch_Schedule.svg"), "Attach spreadsheet", menu
        )
        QtCore.QObject.connect(
            action1, QtCore.SIGNAL("triggered()"), self.attachSpreadsheet
        )
...
class ArchScheduleTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        # connect slots
        QtCore.QObject.connect(
            self.form.buttonAdd, QtCore.SIGNAL("clicked()"), self.add
        )
        QtCore.QObject.connect(
            self.form.buttonDel, QtCore.SIGNAL("clicked()"), self.remove
        )
        QtCore.QObject.connect(
            self.form.buttonClear, QtCore.SIGNAL("clicked()"), self.clear
        )
        QtCore.QObject.connect(
            self.form.buttonImport, QtCore.SIGNAL("clicked()"), self.importCSV
        )
        QtCore.QObject.connect(
            self.form.buttonExport, QtCore.SIGNAL("clicked()"), self.export
        )
        QtCore.QObject.connect(
            self.form.buttonSelect, QtCore.SIGNAL("clicked()"), self.select
        )
        QtCore.QObject.connect(
            self.form.buttonBox, QtCore.SIGNAL("accepted()"), self.accept
        )
        QtCore.QObject.connect(
            self.form.buttonBox, QtCore.SIGNAL("rejected()"), self.reject
        )
            ...
            for i in range(5):
                for j in range(len(obj.Description)):
                    item = QtGui.QTableWidgetItem(
                        [obj.Description, obj.Value, obj.Unit, obj.Objects, obj.Filter][
                            i
                        ][j]
                    )
        ...
        self.form.setWindowFlags(
            self.form.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
        )
        ...
        if filename:
            ...
            if filename.lower().endswith(".csv"):
                ...
                FreeCAD.Console.PrintError(
                    translate("Arch", "Unable to recognize that file type")
                    + ":"
                    + filename
                    + "\n"
                )
        ...
        if self.form.list.currentRow() >= 0:
            ...
            for o in FreeCADGui.Selection.getSelection():
                if o != self.obj:
                    ...
                    sel += o.Name
            if sel:
                self.form.list.setItem(
                    self.form.list.currentRow(), 3, QtGui.QTableWidgetItem(sel)
                )
    ...
    def writeValues(self):
        ...
        if not self.obj:
            self.obj = FreeCAD.ActiveDocument.addObject(
                "App::FeaturePython", "Schedule"
            )
