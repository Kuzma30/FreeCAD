    ...
    if joinArch:
        ...
        for k, v in shtypes.items():
            ...
            if v1.Solids:
                ...
                print(
                    "ArchSectionPlane: Fusing Arch objects produced non-solid results"
                )
                shapes.append(v1)
...
def getSVG(
    ...
    for o in objs:
        if Draft.getType(o) == "Space":
            ...
            nonspaces.append(o)
        if (
            Draft.getType(o.getLinkedObject()) == "Window"
        ):  # To support Link of Windows(Doors)
            windows.append(o)
    objs = nonspaces
    ...
    if renderMode in ["Coin", 2, "Coin mono", 3]:
        ...
        if should_update_svg_cache:
            if renderMode in ["Coin mono", 3]:
                svgcache = getCoinSVG(
                    ...
                    objs,
                    cameradata,
                    linewidth="SVGLINEWIDTH",
                    facecolor="#ffffff",
                )
            else:
                svgcache = getCoinSVG(
                    cutplane, objs, cameradata, linewidth="SVGLINEWIDTH"
                )
        ...
        if should_update_svg_cache:
            ...
            svgcache += render.getSectionSVG(
                linewidth="SVGCUTLINEWIDTH", fillpattern="#ffffff"
            )
            if showHidden:
        ...
        if (
            ...
            if showFill:
                (
                    ...
                    objs, cutplane, onlySolids, clip, joinArch, showHidden, True
                )
            else:
                (
                    vshapes,
    ...
    if cutface and windows:
        cutwindows = [
            w.Name for w in windows if w.Shape.BoundBox.intersect(cutface.BoundBox)
        ]
    if windows:
        sh = []
        for w in windows:
...
def getDXF(obj):
    ...
    objs = [
        ...
        for o in objs
        if (
            (not (Draft.getType(o) in ["Space", "Dimension", "Annotation"]))
...
def getCoinSVG(
    cutplane, objs, cameradata=None, linewidth=0.2, singleface=False, facecolor=None
):
    ...
    clip.on = True
    plane = coin.SbPlane(
        coin.SbVec3f(norm.x, norm.y, norm.z), dist
    )  # dir, position on dir
    ...
    coords.point.setValues(
        [
            ...
            [
                boundbox.XMin + markervec.x,
                boundbox.YMin + markervec.y,
                boundbox.ZMin + markervec.z,
            ],
        ]
    )
    sep.addChild(coords)
    ...
    if factor:
        if trans:
            ...
            svg = svg.replace(
                "<g>",
                '<g transform="scale(' + str(factor) + "," + str(factor) + ')">\n<g>',
                1,
            )
...
class _CommandSectionPlane:
    ...
    def Activated(self):
        ...
        FreeCAD.ActiveDocument.openTransaction(
            translate("Arch", "Create Section Plane")
        )
    ...
    def execute(self, obj):
        ...
        if (
            p.normalAt(0, 0).getAngle(
                obj.Placement.Rotation.multVec(FreeCAD.Vector(0, 0, 1))
            )
            > 1
        ):
            p.reverse()
        ...
        if prop == "LineColor":
            ...
            if hasattr(vobj, "CutView") and FreeCADGui.ActiveDocument.ActiveView:
                ...
                if vobj.CutView:
                    ...
                    dist = (
                        mp.Length
                    )  # - 0.1 # to not clip exactly on the section object
                    norm = norm.negative()
                    marg = 1
    ...
    def setProperties(self, obj):
        ...
        if not "LineColor" in pl:
            obj.addProperty(
                ...
                "Drawing view",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The line color of the projected objects"
                ),
            )
        if not "FillColor" in pl:
            obj.addProperty(
                ...
                "Drawing view",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The color of the cut faces (if turned on)"
                ),
            )
...
class SectionPlaneTaskPanel:
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            self.rotateXButton, QtCore.SIGNAL("clicked()"), self.rotateX
        )
        QtCore.QObject.connect(
            self.rotateYButton, QtCore.SIGNAL("clicked()"), self.rotateY
        )
        QtCore.QObject.connect(
            self.rotateZButton, QtCore.SIGNAL("clicked()"), self.rotateZ
        )
        QtCore.QObject.connect(
            self.resizeButton, QtCore.SIGNAL("clicked()"), self.resize
        )
        QtCore.QObject.connect(
            self.recenterButton, QtCore.SIGNAL("clicked()"), self.recenter
        )
        QtCore.QObject.connect(
            self.tree, QtCore.SIGNAL("itemSelectionChanged()"), self.onTreeClick
        )
        self.update()
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.addButton.setText(
            QtGui.QApplication.translate("Arch", "Add selected", None)
        )
        self.addButton.setToolTip(
            QtGui.QApplication.translate(
                "Arch",
                "Add selected object(s) to the scope of this section plane",
                None,
            )
        )
        self.title.setText(
        ...
        self.rlabel.setText(
            QtGui.QApplication.translate("Arch", "Section plane placement:", None)
        )
        ...
        self.rotateXButton.setToolTip(
            QtGui.QApplication.translate(
                "Arch", "Rotates the plane along the X axis", None
            )
        )
        ...
        self.rotateYButton.setToolTip(
            QtGui.QApplication.translate(
                "Arch", "Rotates the plane along the Y axis", None
            )
        )
        ...
        self.rotateZButton.setToolTip(
            QtGui.QApplication.translate(
                "Arch", "Rotates the plane along the Z axis", None
            )
        )
