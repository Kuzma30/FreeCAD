...
def makeSolarDiagram(longitude, latitude, scale=1, complete=False, tz=None):
    ...
    try:
        ...
        loc = ladybug.location.Location(
            latitude=latitude, longitude=longitude, time_zone=tz
        )
...
def makeWindRose(epwfile, scale=1, sectors=24):
    ...
    sectorangles = [
        i * baseangle for i in range(sectors)
    ]  # the divider angles between each sector
    ...
    windsbysector = [
        0 for i in range(sectors)
    ]  # prepare a holder for values for each sector
    for hour in epw_data.wind_direction:
        sector = min(
            angles, key=lambda x: abs(x - hour)
        )  # find the closest sector angle
...
class _CommandSite:
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Arch_Site", "Creates a site object including selected objects."
            ),
        }
    ...
    def Activated(self):
        ...
        warning = False
        for obj in sel:
            if (Draft.getType(obj) == "Building") or (
                hasattr(obj, "IfcType") and obj.IfcType == "Building"
            ):
                siteobj.append(obj)
            else:
                if link == True:
    ...
    def setProperties(self, obj):
        ...
        if not "PostalCode" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The postal or zip code of this site"
                ),
            )
        if not "City" in pl:
            obj.addProperty(
        ...
        if not "Region" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The region, province or county of this site"
                ),
            )
        ...
        if not "Elevation" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The elevation of level 0 of this site"
                ),
            )
        if not "Url" in pl:
            obj.addProperty(
        ...
        if not "Perimeter" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The perimeter length of this terrain"
                ),
            )
        ...
        if not "RemoveSplitter" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "Remove splitters from the resulting shape"
                ),
            )
        ...
        if not "TimeZone" in pl:
            obj.addProperty(
                ...
                "Site",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The time zone where this site is located"
                ),
            )
                    ...
                    if sub.Shape:
                        if sub.Shape.Solids:
                            for sol in sub.Shape.Solids:
                                ...
                                shells.append(
                                    sol.Shells[0].common(
                                        shape.extrude(obj.ExtrusionVector)
                                    )
                                )
                                shape = rest
    ...
    def computeAreas(self, obj):
        ...
        if fset:
            ...
            for f in fset:
                try:
                    pf = Part.Face(
                        Part.Wire(TechDraw.project(f, FreeCAD.Vector(0, 0, 1))[0].Edges)
                    )
...
class _ViewProviderSite:
    ...
    def claimChildren(self):
        ...
        if hasattr(self, "Object"):
            ...
            if hasattr(self.Object, "Additions") and prefs.GetBool(
                "swallowAdditions", True
            ):
                objs.extend(self.Object.Additions)
            if hasattr(self.Object, "Subtractions") and prefs.GetBool(
                "swallowSubtractions", True
            ):
            ...
            if hasattr(vobj, "SolarDiagram") and hasattr(vobj, "SolarDiagramScale"):
                if vobj.SolarDiagram:
                    ...
                    self.diagramnode = makeSolarDiagram(
                        vobj.Object.Longitude,
                        vobj.Object.Latitude,
    ...
    def updateTrueNorthRotation(self):
        ...
        if hasattr(self, "trueNorthRotation") and self.trueNorthRotation is not None:
            ...
            self.trueNorthRotation.rotation.setValue(
                coin.SbVec3f(0, 0, 1), math.radians(-angle)
            )
