...
def makeStairs(
    baseobj=None, length=None, width=None, height=None, steps=None, name="Stairs"
):
    ...
    def makeRailingLorR(stairs, side="L"):
        for stair in reversed(stairs):
            if side == "L":
                ...
                stairs0RailingLR = (
                    "RailingLeft"  # stairs0OutlineWireLR = "OutlineWireLeft"
                )
                ...
                stairs0RailingLR = (
                    "RailingRight"  # stairs0OutlineWireLR = "OutlineWireRight"
                )
                stairRailingLR = (
                    "RailingRight"  # stairOutlineWireLR = "OutlineWireRight"
                )
            if outlineLR or outlineLRAll:
                lrRail = ArchPipe.makePipe(
                    baseobj=None, diameter=0, length=0, placement=None, name="Rail"
                )
                if outlineLRAll:
                    # lrRail.Base = lrRailWire					# no need to set here as _Stairs will do
                    setattr(
                        stair, stairRailingLR, lrRail.Name
...
class _Stairs(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not hasattr(obj, "RailingLeft"):
            ...
            if hasattr(obj, "OutlineWireLeft"):
                outlineWireLeftObject = FreeCAD.ActiveDocument.getObject(
                    obj.OutlineWireLeft
                )
                try:
        ...
        if not hasattr(obj, "RailingRight"):
            ...
            if hasattr(obj, "OutlineWireRight"):
                outlineWireRightObject = FreeCAD.ActiveDocument.getObject(
                    obj.OutlineWireRight
                )
                try:
        ...
        if not "ConnectionDownStartStairs" in pl:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The type of connection between the lower slab and the start of the stairs",
                ),
            )
    ...
    def execute(self, obj):
        ...
        if obj.RailingLeft:
            ...
            if (
                railingLeftObject
            ):  # TODO - need to update if railing is deleted by user? This become None if deleted.
                ...
                if railWireL:
                    # Migration
                    if (
                        Draft.getType(railingLeftObject.Base) != "Part"
                    ):  # None or not "Part"
                        railingLeftWireObject = FreeCAD.ActiveDocument.addObject(
                            "Part::Feature", "RailingWire"
                        )
                        if (
                            railingLeftObject.Base
                        ):  # if has railingLeftObject.Base but that != "Part"
                            railingLeftObject.Document.removeObject(
                                railingLeftObject.Base.Name
                            )  # Delete the previous Base object... # Not Using FreeCAD.ActiveDocument...
            ...
            if (
                ...
                if railWireR:
                    ...
                    if Draft.getType(railingRightObject.Base) != "Part":
                        railingRightWireObject = FreeCAD.ActiveDocument.addObject(
                            "Part::Feature", "RailingWire"
                        )
                        if railingRightObject.Base:
                            railingRightObject.Document.removeObject(
                                railingRightObject.Base.Name
                            )
        ...
        if obj.Structure == "Massive":
            ...
            if obj.StringerWidth.Value and obj.StructureThickness.Value:
                ...
                if obj.TreadThickness.Value and p5:
                    ...
                    p5c = p5b.add(DraftVecUtils.scaleTo(vLength, -reslength)).add(
                        Vector(0, 0, -obj.TreadThickness.Value)
                    )
                    pol = Part.Face(
                        Part.makePolygon([p1c, p1b, p2, p3, p4, p5, p5b, p5c, p1c])
                    )
                else:
    ...
    def makeStraightStairsWithLanding(self, obj, edge):
        ...
        print(p1, p2)
        if obj.Landings == "At center" and obj.Flight not in [
            "HalfTurnLeft",
            "HalfTurnRight",
        ]:
            print(p3, p4)
        elif obj.Landings == "At center" and obj.Flight in [
            "HalfTurnLeft",
            "HalfTurnRight",
        ]:
            print(p3r, p4r)
