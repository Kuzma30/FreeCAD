...
def makeWall(
    baseobj=None, height=None, length=None, width=None, align=None, face=None, name=None
):
    ...
    if baseobj:
        if hasattr(baseobj, "Shape") or baseobj.isDerivedFrom("Mesh::Feature"):
            ...
            FreeCAD.Console.PrintWarning(
                str(
                    translate("Arch", "Walls can only be based on Part or Mesh objects")
                )
            )
    if face:
        obj.Face = face
    ...
    for att in ["Width", "Height", "Align"]:
        ...
        for w in walls:
            ...
            if not value:
                ...
                if type(value) == float:
                    if round(value, Draft.precision()) != round(
                        getattr(w, att), Draft.precision()
                    ):
                        return False
                else:
...
class _CommandWall:
    ...
    def taskbox(self):
        ...
        value5.setChecked(
            FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetBool(
                "WallSketches", True
            )
        )
        ...
        QtCore.QObject.connect(
            value1, QtCore.SIGNAL("valueChanged(double)"), self.setWidth
        )
        QtCore.QObject.connect(
            value2, QtCore.SIGNAL("valueChanged(double)"), self.setHeight
        )
        QtCore.QObject.connect(
            value3, QtCore.SIGNAL("currentIndexChanged(int)"), self.setAlign
        )
        QtCore.QObject.connect(
            value4, QtCore.SIGNAL("stateChanged(int)"), self.setContinue
        )
        QtCore.QObject.connect(
            value5, QtCore.SIGNAL("stateChanged(int)"), self.setUseSketch
        )
        QtCore.QObject.connect(
            self.Length, QtCore.SIGNAL("returnPressed()"), value1.setFocus
        )
        QtCore.QObject.connect(
            self.Length, QtCore.SIGNAL("returnPressed()"), value1.selectAll
        )
        QtCore.QObject.connect(
            value1, QtCore.SIGNAL("returnPressed()"), value2.setFocus
        )
        QtCore.QObject.connect(
            value1, QtCore.SIGNAL("returnPressed()"), value2.selectAll
        )
        QtCore.QObject.connect(
            value2, QtCore.SIGNAL("returnPressed()"), self.createFromGUI
        )
        QtCore.QObject.connect(
            matCombo, QtCore.SIGNAL("currentIndexChanged(int)"), self.setMat
        )
        return w
    ...
    def setAlign(self, i):
        ...
        FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").SetInt(
            "WallAlignment", i
        )
    ...
    def setUseSketch(self, i):
        ...
        FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").SetBool(
            "joinWallSketches", bool(i)
        )
...
class _CommandMergeWalls:
    ...
    def Activated(self):
        ...
        if len(walls) == 1:
            if Draft.getType(walls[0]) == "Wall":
                ...
                if ok:
                    ...
                    FreeCAD.Console.PrintWarning(
                        translate(
                            "Arch", "The selected wall contains no subwall to merge"
                        )
                    )
                    return
            else:
                FreeCAD.Console.PrintWarning(
                    translate("Arch", "Please select only wall objects")
                )
                return
        for w in walls:
        ...
        FreeCADGui.addModule("Arch")
        FreeCADGui.doCommand(
            "Arch.joinWalls(FreeCADGui.Selection.getSelection(),delete=True)"
        )
        FreeCAD.ActiveDocument.commitTransaction()
    ...
    def setProperties(self, obj):
        ...
        if not "OverrideWidth" in lp:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "This overrides Width attribute to set width of each segment of wall.  Ignored if Base object provides Widths information, with getWidths() method.  (The 1st value override 'Width' attribute for 1st segment of wall; if a value is zero, 1st value of 'OverrideWidth' will be followed)",
                ),
        ...
        if not "OverrideAlign" in lp:
            obj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "This overrides Align attribute to set Align of each segment of wall.  Ignored if Base object provides Aligns information, with getAligns() method.  (The 1st value override 'Align' attribute for 1st segment of wall; if a value is not 'Left, Right, Center', 1st value of 'OverrideAlign' will be followed)",
                ),
            )  # see DraftGeomUtils.offsetwire()
                        ...
                        if DraftGeomUtils.geomType(e) == "Line":
                            if e.Length != obj.Length.Value:
                                ...
                                if Draft.getType(obj.Base) == "Wire":
                                    ...
                                    Draft.getType(obj.Base) == "Sketcher::SketchObject"
                                ):
                                    try:
...
class _ViewProviderWall(ArchComponent.ViewProviderComponent):
    ...
    def attach(self, vobj):
        ...
        s = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetFloat(
            "patternScale", 0.01
        )
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        action1 = QtGui.QAction(
            QtGui.QIcon(":/icons/Arch_Wall_Tree.svg"), "Flip direction", menu
        )
        QtCore.QObject.connect(
            action1, QtCore.SIGNAL("triggered()"), self.flipDirection
        )
