...
class _CommandWindow:
    ...
    def getPoint(self, point=None, obj=None):
        ...
        if self.baseFace is not None:
            FreeCADGui.doCommand(
                "pl = WorkingPlane.getPlacementFromFace(FreeCAD.ActiveDocument."
    ...
    def taskbox(self):
        ...
        QtCore.QObject.connect(
            include, QtCore.SIGNAL("stateChanged(int)"), self.setInclude
        )
        ...
        QtCore.QObject.connect(
            values, QtCore.SIGNAL("valueChanged(double)"), self.setSill
        )
        ...
        for param in self.wparams:
            ...
            if param == "Width":
                ...
                ).GetFloat("WindowO1", 0.0)
                wid.setText(FreeCAD.Units.Quantity(n, FreeCAD.Units.Length).UserString)
                setattr(self, param, n)
            elif param == "W1":
                n = FreeCAD.ParamGet(
                    "User parameter:BaseApp/Preferences/Mod/Arch"
                ).GetFloat("WindowW1", self.Thickness * 2)
                wid.setText(FreeCAD.Units.Quantity(n, FreeCAD.Units.Length).UserString)
                setattr(self, param, n)
            else:
                n = FreeCAD.ParamGet(
                    "User parameter:BaseApp/Preferences/Mod/Arch"
                ).GetFloat("Window" + param, self.Thickness)
                wid.setText(FreeCAD.Units.Quantity(n, FreeCAD.Units.Length).UserString)
                setattr(self, param, n)
        ...
        if self.doormode:
            i = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetInt(
                "DoorPreset", 0
            )
            d = FreeCAD.ParamGet(
                "User parameter:BaseApp/Preferences/Mod/Arch"
...
class _Window(ArchComponent.Component):
    ...
    def getSubVolume(self, obj, plac=None):
        ...
        if hasattr(
            obj, "HoleDepth"
        ):  # the code have not checked whether this is a clone and use the original's HoleDepth; if HoleDepth is set in this object, even it is a clone, the original's HoleDepth is overridden
        ...
        if not width:
            if Draft.isClone(
                obj, "Window"
            ):  # check whether this is a clone and use the original's HoleDepth or Shape's Boundbox
        ...
        if not base:
            if Draft.isClone(
                obj, "Window"
            ):  # if this object has not base, check whether this is a clone and use the original's base
                if hasattr(obj, "CloneOf"):
                    orig = obj.CloneOf
                else:
        ...
        if hasattr(
            obj, "HoleWire"
        ):  # the code have not checked whether this is a clone and use the original's HoleWire; if HoleWire is set in this object, even it is a clone, the original's BoundBox/HoleWire is overridden
            if obj.HoleWire > 0:
...
class _ViewProviderWindow(ArchComponent.ViewProviderComponent):
                ...
                if hasattr(obj.Material, "Materials"):
                    if obj.Material.Names:
                        ...
                        if name in obj.Material.Names:
                            ...
                            mat = obj.Material.Materials[
                                obj.Material.Names.index(mtype)
                            ]
                        if mat:
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        if hasattr(self, "Object"):
            ...
            QtCore.QObject.connect(
                action1, QtCore.SIGNAL("triggered()"), self.invertOpening
            )
            menu.addAction(action1)
            action2 = QtGui.QAction(
                QtGui.QIcon(":/icons/Arch_Window_Tree.svg"),
                "Invert hinge position",
                menu,
            )
            QtCore.QObject.connect(
                action2, QtCore.SIGNAL("triggered()"), self.invertHinge
            )
...
class _ArchWindowTaskPanel:
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.addp4.setToolTip(
            QtGui.QApplication.translate(
                "Arch",
                "If this is checked, the default Frame value of this window will be added to the value entered here",
                None,
            )
        )
        ...
        self.addp5.setToolTip(
            QtGui.QApplication.translate(
                "Arch",
                "If this is checked, the default Offset value of this window will be added to the value entered here",
                None,
            )
        )
        self.field6.setText(
