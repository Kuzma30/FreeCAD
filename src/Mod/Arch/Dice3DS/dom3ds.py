        ...
        while fbuf.room_for_chunks():
            ...
            if label in self.single_types:
                ...
                if flags["tight"] and getattr(self, attr) is not None:
                    raise File3dsFormatError("Chunk %s appeared twice" % label, True)
                ...
                raise File3dsFormatError("Illegal context for chunk %s" % label, True)
...
class OneColorChunk(ChunkBase):
    single = (
        "COLOR_F color, COLOR_24 color," "LIN_COLOR_F lincolor, LIN_COLOR_24 lincolor"
    )
...
class SMAGIC(UndefinedChunk):
    tag = 0x2D2D


class LMAGIC(UndefinedChunk):
    tag = 0x2D3D


class MLIBMAGIC(UndefinedChunk):
    tag = 0x2DAA


class PRJMAGIC(UndefinedChunk):
    tag = 0x3D2C


class MATMAGIC(UndefinedChunk):
    tag = 0x3DFF
...
class V_GRADIENT(ChunkBase):
    ...
    multiple = (
        "COLOR_F color, LIN_COLOR_F lincolor," "COLOR_24 color, LIN_COLOR_24 lincolor"
    )
...
class FOG(ChunkBase):
    tag = 0x2200
    struct = (
        "float near_plane, float near_density," "float far_plane, float far_density"
    )
...
class LAYER_FOG(ChunkBase):
    tag = 0x2302
    struct = "float fog_z_from, float fog_z_to," "float fog_density, long fog_type"
...
class DEFAULT_VIEW(ChunkBase):
    tag = 0x3000
    single = (
        "VIEW_TOP view, VIEW_BOTTOM view, VIEW_LEFT view,"
...
class ViewChunk(ChunkBase):
    struct = "float target_x, float target_y," "float target_z, float view_width"
...
class POINT_ARRAY(ArrayChunk):
    ...
    def dump_array(self, flo, indent, flags):
        ...
        for i in xrange(n):
            flo.write(
                "%s    %12.4g%12.4g%12.4g\n"
                % (indent, self.array[i, 0], self.array[i, 1], self.array[i, 2])
            )
        if n < self.npoints:
            flo.write("%s            ...\n" % indent)
...
class MESH_TEXTURE_INFO(MatrixChunk):
    ...
    struct = (
        "float x_tiling, float y_tiling, float icon_x,"
...
class MSH_BOXMAP(ChunkBase):
    tag = 0x4190
    struct = (
        "string front, string back, string left,"
...
class DL_SPOTLIGHT(ChunkBase):
    tag = 0x4610
    struct = (
        "float spot_x, float spot_y, float spot_z,"
        "float hotspot_angle, float falloff_angle"
    )
    single = (
        "DL_SPOT_ROLL roll_angle,"
...
class VIEWPORT_LAYOUT(ChunkBase):
    tag = 0x7001
    struct = (
        "short form, short top, short ready,"
...
class ViewportDataChunk(ChunkBase):
    struct = (
        "short flags, short axis_lockout, short win_x,"
...
class MAT_ACUBIC(ChunkBase):
    tag = 0xA310
    struct = "byte shade, byte antialias, short rflags," "long mapsize, long frame"
...
class MAT_SXP_TEXT_DATA(UndefinedChunk):
    tag = 0xA320


class MAT_SXP_TEXT2_DATA(UndefinedChunk):
    tag = 0xA321


class MAT_SXP_OPAC_DATA(UndefinedChunk):
    tag = 0xA322


class MAT_SXP_BUMP_DATA(UndefinedChunk):
    tag = 0xA324


class MAT_SXP_SPEC_DATA(UndefinedChunk):
    tag = 0xA325


class MAT_SXP_SHIN_DATA(UndefinedChunk):
    tag = 0xA326


class MAT_SXP_SELFI_DATA(UndefinedChunk):
    tag = 0xA328


class MAT_SXP_TEXT_MASKDATA(UndefinedChunk):
    tag = 0xA32A


class MAT_SXP_TEXT2_MASKDATA(UndefinedChunk):
    tag = 0xA32C


class MAT_SXP_OPAC_MASKDATA(UndefinedChunk):
    tag = 0xA32E


class MAT_SXP_BUMP_MASKDATA(UndefinedChunk):
    tag = 0xA330


class MAT_SXP_SPEC_MASKDATA(UndefinedChunk):
    tag = 0xA332


class MAT_SXP_SHIN_MASKDATA(UndefinedChunk):
    tag = 0xA334


class MAT_SXP_SELFI_MASKDATA(UndefinedChunk):
    tag = 0xA336
...
class TARGET_NODE_TAG(ChunkBase):
    tag = 0xB004
    single = "NODE_ID node_id," "NODE_HDR node_hdr," "POS_TRACK_TAG pos_track"
...
class VIEWPORT_LAYOUT_OLD(UndefinedChunk):
    tag = 0x7000


class VIEWPORT_DATA_OLD(UndefinedChunk):
    tag = 0x7010


class OLD_MAT_GROUP(UndefinedChunk):
    tag = 0x4131


class MAT_MAP_TILING_OLD(UndefinedChunk):
    tag = 0xA350


class MAT_MAP_TEXBLUR_OLD(UndefinedChunk):
    tag = 0xA352
