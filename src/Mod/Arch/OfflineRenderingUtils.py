...
def render(
    ...
    offscreenRenderer.setBackgroundColor(
...
def getCoinCamera(camerastring):
    ...
    if camerastring:
        ...
        for child in node.getChildren():
            if ("SoOrthographicCamera" in str(child)) or (
                "SoPerspectiveCamera" in str(child)
            ):
                return child
            ...
            if prop["type"] in ["App::PropertyString", "PropertyFont"]:
                ...
                if len(hex_repr) > 8:
                    raise NotImplementedError(
                        ...
                        "specify number of colors in 4 bytes.".format(
                            len(prop["value"])
                        )
                    )
