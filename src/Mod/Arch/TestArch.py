...
class ArchTest(unittest.TestCase):
    ...
    def testRebar(self):
        ...
        sk.Support = (s, ["Face6"])
        sk.addGeometry(
            Part.LineSegment(App.Vector(-0.85, 1.25, 0), App.Vector(0.75, 1.25, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(0.75, 1.25, 0), App.Vector(0.75, -1.20, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(0.75, -1.20, 0), App.Vector(-0.85, -1.20, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(-0.85, -1.20, 0), App.Vector(-0.85, 1.25, 0))
        )
    ...
    def testWindow(self):
        ...
        sk.Support = (w, ["Face3"])
        sk.addGeometry(
            Part.LineSegment(App.Vector(-1.80, 1.30, 0), App.Vector(-0.90, 1.30, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(-0.90, 1.30, 0), App.Vector(-0.90, 0.25, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(-0.90, 0.25, 0), App.Vector(-1.80, 0.25, 0))
        )
        sk.addGeometry(
            Part.LineSegment(App.Vector(-1.80, 0.25, 0), App.Vector(-1.80, 1.30, 0))
        )
        sk.addConstraint(Sketcher.Constraint("Coincident", 0, 2, 1, 1))
        sk.addConstraint(Sketcher.Constraint("Coincident", 1, 2, 2, 1))
        sk.addConstraint(Sketcher.Constraint("Coincident", 2, 2, 3, 1))
    ...
    def testRoof81Permutations(self):
        ...
        for iY in range(9):
            for iX in range(9):
                ...
                roof = Arch.makeRoof(
                    wire, angles=angsLst, run=runsLst, overhang=overhangsLst
                )
                roof.recompute()
