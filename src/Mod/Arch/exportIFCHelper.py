...
def writeUnits(ifcfile, unit="metre"):
    ...
    if unit == "foot":
        ...
        d2 = ifcfile.createIfcMeasureWithUnit(
            ifcfile.createIfcRatioMeasure(0.3048), ifcfile[13]
        )
        ...
        ifcfile.createIfcUnitAssignment(
            (ifcfile[13], ifcfile[14], ifcfile[15], ifcfile[18])
        )
    return ifcfile
...
class SIUnitCreator:
    def __init__(self, file, text, type):
        ...
        self.text = text
        self.SIUnit = file.createIfcSIUnit(
            None, type, self.getSIPrefix(), self.getSIUnitName()
        )
...
class ContextCreator:
    ...
    def createTargetCRS(self):
        try:
            SIUnit = SIUnitCreator(
                self.file, self.project_data["map_unit"], "LENGTHUNIT"
            )
...
class recycler:
    ...
    def __init__(self, ifcfile):
        ...
        self.compress = FreeCAD.ParamGet(
            "User parameter:BaseApp/Preferences/Mod/Arch"
        ).GetBool("ifcCompress", True)
        self.mergeProfiles = FreeCAD.ParamGet(
            "User parameter:BaseApp/Preferences/Mod/Arch"
        ).GetBool("ifcMergeProfiles", False)
    ...
    def createIfcSurfaceStyleRendering(self, col, trans=0):
        ...
        if self.compress and key in self.ssrenderings:
            ...
            c = self.ifcfile.createIfcSurfaceStyleRendering(
                col, trans, None, None, None, None, None, None, "FLAT"
            )
            if self.compress:
                self.ssrenderings[key] = c
            return c

    def createIfcCartesianTransformationOperator3D(
        self, axis1, axis2, origin, scale, axis3
    ):
        key = (
