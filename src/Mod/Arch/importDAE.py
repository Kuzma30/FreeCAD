...
def checkCollada():
    ...
    try:
        ...
        FreeCAD.Console.PrintError(
            translate("Arch", "pycollada not found, collada support is disabled.")
            + "\n"
        )
        return False
    else:
...
def export(exportList, filename, tessellation=1, colors=None):
    ...
    for obj in objectslist:
        ...
        vert_src = collada.source.FloatSource(
            "cubeverts-array" + str(objind), vindex, ("X", "Y", "Z")
        )
        normal_src = collada.source.FloatSource(
            "cubenormals-array" + str(objind), nindex, ("X", "Y", "Z")
        )
        geom = collada.geometry.Geometry(
            colmesh, "geometry" + str(objind), obj.Name, [vert_src, normal_src]
        )
                ...
                if hasattr(obj.Material, "Material"):
                    if "DiffuseColor" in obj.Material.Material:
                        kd = tuple(
                            [
                                float(k)
                                for k in obj.Material.Material["DiffuseColor"]
                                .strip("()")
                                .split(",")
                            ]
                        )
                        effect = collada.material.Effect(
                            "effect_" + obj.Material.Name,
                            [],
                            "phong",
                            diffuse=kd,
                            specular=(1, 1, 1),
                        ...
                        mat = collada.material.Material(
                            "mat_" + obj.Material.Name, obj.Material.Name, effect
                        )
        ...
        if not matnode:
            if not defaultmat:
                ...
                defaultmat = collada.material.Material(
                    "mat_default", "default_material", effect
                )
                colmesh.effects.append(effect)
    ...
    FreeCAD.Console.PrintMessage(
        translate("Arch", "file %s successfully created.") % filename
    )
