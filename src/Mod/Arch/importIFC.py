...
def getPreferences():
    ...
    preferences = {
        ...
        "SPLIT_LAYERS": p.GetBool(
            "ifcSplitLayers", False
...
def insert(srcfile, docname, skip=[], only=[], root=None, preferences=None):
    ...
    for product in products:
        ...
        archobj = (
            True  # assume all objects not in structuralifcobjects are architecture
        )
        structobj = False
            ...
            if shape.isNull() and (not preferences["ALLOW_INVALID"]):
                ...
                if (preferences["MERGE_MODE_ARCH"] > 0 and archobj) or structobj:
                    ...
                    if clone:
                        ...
                        if preferences["GET_EXTRUSIONS"] and (
                            preferences["MERGE_MODE_ARCH"] != 1
                        ):
                            ...
                            if not sortmethod:
                                if ptype in [
                                    ...
                                    sortmethod = "area"
                            ex = Arch.getExtrusionData(
                                shape, sortmethod
                            )  # is this an extrusion?
                            if ex:
                                ...
                                if addplacement:
                                    ...
                                    baseobj.Dir = (
                                        addplacement.Rotation.inverted().multVec(ex[1])
                                    )
                                else:
                                    baseobj.Dir = ex[1]
                                if FreeCAD.GuiUp:
        ...
        if preferences["MERGE_MODE_ARCH"] == 0 and archobj:
            ...
            for freecadtype, ifctypes in typesmap.items():
                ...
                if clone:
                    ...
                    # no clone
                    obj = getattr(Arch, "make" + freecadtype)(
                        baseobj=baseobj, name=name
                    )
                    if (
            ...
            if hasattr(obj, "IfcData"):
                ...
                for attribute in ArchIFCSchema.IfcProducts[product.is_a()][
                    "attributes"
                ]:
                    ...
                    if (
                        ...
                        setattr(
                            obj, attribute["name"], getattr(product, attribute["name"])
                        )
        ...
        if preferences["DEBUG"]:
            print(
                ""
            )  # newline for debug prints, print for a new object should be on a new line
        ...
        if obj:
            ...
            if psets:
                ...
                if preferences["IMPORT_PROPERTIES"] and hasattr(obj, "IfcProperties"):
                        ...
                        for p in psets[c]:
                            ...
                            if l.is_a("IfcPropertySingleValue"):
                                ...
                                if l.NominalValue:
                                    ...
                                    ifc_spreadsheet.set(
                                        str("C" + str(n)), l.NominalValue.is_a()
                                    )
                                    if l.NominalValue.is_a() in [
                                        ...
                                        if six.PY2:
                                            ifc_spreadsheet.set(
                                                ...
                                                "'"
                                                + str(
                                                    l.NominalValue.wrappedValue.encode(
                                                        "utf8"
                                                    )
                                                ),
                                            )
                ...
                elif hasattr(obj, "IfcProperties") and isinstance(
                    ...
                    d = obj.IfcProperties
                    obj.IfcProperties = importIFCHelper.getIfcProperties(
                        ifcfile, pid, psets, d
                    )
            ...
            if product.is_a("IfcSite"):
                ...
                if modelRC and modelRC.TrueNorth:
                    ...
                    obj.Declination = (
                        (math.degrees(math.atan2(y, x)) - 90 + 180) % 360
                    ) - 180
                    if FreeCAD.GuiUp:
    ...
    if preferences["MERGE_MODE_STRUCT"] == 2:
            ...
            if ifcfile[host].is_a("IfcStructuralAnalysisModel"):
                ...
                if host in objects.keys():
                    ...
                    for c in childs_to_delete:
                        children.remove(
                            c
                        )  # to not process the child again in remaining groups
                    if cobs:
                        if preferences["DEBUG"]:
                            print(
                                "adding ",
        ...
        if (
            preferences["MERGE_MODE_ARCH"] > 2
        ):  # if ArchObj is compound or ArchObj not imported
            doc.recompute()
    ...
    if preferences["MERGE_MODE_ARCH"] == 3:
        ...
        if preferences["SEPARATE_OPENINGS"]:
            for subtraction in subtractions:
                if (subtraction[0] in objects.keys()) and (
                    subtraction[1] in objects.keys()
                ):
                    ...
                    Arch.removeComponents(
                        objects[subtraction[0]], objects[subtraction[1]]
                    )
                    if preferences["DEBUG"]:
                        doc.recompute()
        ...
        for host, children in additions.items():
            ...
            for child in children:
                if (
                    child in objects.keys() and child not in swallowed
        ...
        for obj in objects.values():
            if obj.isDerivedFrom("Part::Feature"):
                if obj.Shape.isNull() and not (
                    Draft.getType(obj) in ["Site", "Project"]
                ):
                    Arch.rebuildArchShape(obj)
    ...
    for material in materials:
        ...
        if mat_color is not None:
            ...
            if preferences["DEBUG"]:
                print(
                    "/n  no color for material: {}, ".format(str(material.id)), end=""
                )
        ...
        if preferences["MERGE_MATERIALS"]:
            for added_mat in added_mats:
                if (
                    ...
                    and "DiffuseColor"
                    in mdict  # Description has been set thus it is in mdict
                    ...
                    if hasattr(objects[o], "Material"):
                        ...
                        if FreeCAD.GuiUp:
                            ...
                            if "DiffuseColor" in objects[o].Material.Material:
                                string_color = objects[o].Material.Material[
                                    "DiffuseColor"
                                ]
                                col = tuple(
                                    [
                                        float(f)
                                        for f in string_color.strip("()").split(",")
                                    ]
                                )
                                ma_color = (
                                    round(col[0], dig),
                            ...
                            if ma_color != sh_color:
                                ...
                                print(
                                    "    material color is used (most software uses shape color)"
                                )
                                print(
                                    "    obj: ",
                                    o,
