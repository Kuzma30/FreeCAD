...
subtractiveTypes = [
    "IfcOpeningElement"
]  # elements that must be subtracted from their parents
SCHEMA = "http://www.steptools.com/support/stdev_docs/ifcbim/ifc4.exp"  # only for internal parser
MAKETEMPFILES = (
    False  # if True, shapes are passed from ifcopenshell to freecad through temp files
)
...
APPLYFIX = (
    True  # if true, the ifcopenshell bug-fixing function is applied when saving files
)
# end config
    ...
    try:
        ...
        try:
            ...
            FreeCAD.Console.PrintMessage(
                translate("Arch", "Couldn't locate IfcOpenShell") + "\n"
            )
            return False
        else:
            IFCOPENSHELL5 = True
...
def read(filename, skip=None):
    ...
    if getIfcOpenShell() and not FORCE_PYTHON_PARSER:
        ...
        if IFCOPENSHELL5:
            ...
            if DEBUG:
                print(
                    "Warning: IfcOpenShell version very old, unable to handle Brep data"
                )
        ...
        while True:
            ...
            # check if object was already processed, to workaround an ifcopenshell bug
                ...
                # spaces
                    ...
                    if DEBUG:
                        print(
                            "Fixme: Shape-containing object not handled: ",
                            objid,
                            " ",
                            objtype,
                        )
                    ...
                    if hasattr(obj, "mesh"):
                        if not hasattr(obj.mesh, "verts"):
                            obj = (
                                IfcImport.Get()
                            )  # Get triangulated rep of same product
                        me, pl = getMesh(obj)
                        ...
                        if DEBUG:
                            print(
                                "Error: Skipping object without mesh: ",
                                objid,
                                " ",
                                objtype,
                            )
            ...
            for c in comps:
                ...
                if (id <= 0) or (parent_id <= 0):
                    ...
                    if parentid <= 0:
                        ...
                        if DEBUG:
                            print(
                                "Fixme: skipping unhandled parent: ",
                                parentid,
                                " ",
                                parenttype,
                            )
                ...
                if parent and (id in ifcObjects):
                    if ifcObjects[id] and (ifcObjects[id].Name != parent.Name):
                        if additive:
                            if DEBUG:
                                print(
                                    "adding ", ifcObjects[id].Name, " to ", parent.Name
                                )
                ...
                for b in r.Items:
                    if b.type == "IFCEXTRUDEDAREASOLID":
                        ...
                        window = Arch.makeWindow(
                            wire, width=b.Depth, name=objtype + str(entity.id)
                        )
        if window:
            if DEBUG:
...
def getMesh(obj):
    ...
    if IFCOPENSHELL5:
        return None, None
        print(
            "fixme: mesh data not yet supported"
        )  # TODO implement this with OCC tessellate
    import Mesh
    ...
    if IFCOPENSHELL5:
        try:
            ...
            if SEPARATE_OPENINGS and hasattr(IfcImport, "DISABLE_OPENING_SUBTRACTIONS"):
                if SEPARATE_PLACEMENTS and hasattr(
                ...
                if SEPARATE_PLACEMENTS and hasattr(
                    ...
                    brep_data = IfcImport.create_shape(
                        obj, IfcImport.DISABLE_OBJECT_PLACEMENT | ss
                    )
                else:
...
def getSchema():
    ...
    custom = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetString(
        "CustomIfcSchema", ""
    )
    if custom:
...
def group(entity, ifc, mode=None):
    ...
    try:
        ...
        subelements = ifc.find(
            "IFCRELCONTAINEDINSPATIALSTRUCTURE", "RelatingStructure", entity
        )
                ...
                for t in g[1]:
                    if e.type.upper() == t.upper():
                        if hasattr(FreeCAD.ActiveDocument, g[0] + str(e.id)):
                            g[2].append(
                                FreeCAD.ActiveDocument.getObject(g[0] + str(e.id))
                            )
        print("groups:", groups)
    ...
    for obj in objectslist:
        ...
        if obj.isDerivedFrom("App::DocumentObjectGroup"):
            ...
            if gdata:
                ...
                if gdata[0] == "polyline":
                    representation = ifc.addExtrudedPolyline(
                        gdata[1], gdata[2], color=color
                    )
                elif gdata[0] == "circle":
                    representation = ifc.addExtrudedCircle(
                        gdata[1], gdata[2], color=color
                    )
                elif gdata[0] == "ellipse":
                    representation = ifc.addExtrudedEllipse(
                        gdata[1], gdata[2], color=color
                    )
                elif gdata[0] == "composite":
                    representation = ifc.addExtrudedCompositeCurve(
                        gdata[1], gdata[2], color=color
                    )
                else:
            ...
            if product:
                ...
                if SEPARATE_OPENINGS and gdata:
                    for o in obj.Subtractions:
                        ...
                        representation = [
                            ifc.addFacetedBrep(f, color=color) for f in fdata
                        ]
                        p2 = ifc.addProduct(
                            "IfcOpeningElement",
                            representation,
    ...
    for name, entities in groups.items():
        if entities:
            ...
            if o:
                if DEBUG:
                    print(
                        "Adding group ", o.Label, " with ", len(entities), " elements"
                    )
            ...
            if (len(p) == 1) and v:
                ...
                for e in p.Edges:
                    if isinstance(e.Curve, Part.Circle):
                        ...
                        print(
                            "Arch.getIfcExtrusionData: Warning: unsupported edge type in profile"
                        )
                if curves:
                    # Composite profile
                    ecurves = []
                    ...
                    for e in edges:
                        if isinstance(e.Curve, Part.Circle):
                            ...
                            p2 = math.degrees(
                                -DraftVecUtils.angle(
                                    e.Vertexes[-1].Point.sub(e.Curve.Center)
                                )
                            )
                            ...
                            if last:
                                ...
                                last = e.Vertexes[-1].Point
                            ecurves.append(
                                ["line", [getTuples(vert, scale) for vert in verts]]
                            )
...
def explore(filename=None):
    ...
    if not filename:
        ...
        filename = QtGui.QFileDialog.getOpenFileName(
            QtGui.QApplication.activeWindow(), "IFC files", "*.ifc"
        )
        if filename:
            filename = filename[0]
    if filename:
...
class IfcSchema:
    ...
    def readEntities(self):
        ...
        for m in re.finditer("ENTITY (.*?)END_ENTITY;", self.data, re.DOTALL):
            ...
            entity["name"] = (
                re.search("(.*?)[;|\s]", raw_entity_str).groups()[0].upper()
            )

            subtypeofmatch = re.search(".*SUBTYPE OF \((.*?)\);", raw_entity_str)
            entity["supertype"] = (
                subtypeofmatch.groups()[0].upper() if subtypeofmatch else None
            )
...
class IfcFile:
    ...
    def __init__(self, filename, schema):
        ...
        if DEBUG:
            print(
                "Parsed from file %s: %s entities" % (self.filename, len(self.entById))
            )
...
class IfcWriter(object):
    ...
    def __init__(
        ...
        self._org = create(
            self._fileobject, "IfcOrganization", [None, "", None, None, None]
        )
        pno = create(
            self._fileobject,
            "IfcPersonAndOrganization",
            [self._person, self._org, None],
        )
        app = create(
            self._fileobject, "IfcApplication", [self._org, version, application, uid()]
        )
        self._owner = create(
            self._fileobject,
            "IfcOwnerHistory",
            [pno, app, None, "ADDED", None, pno, app, now()],
        )
        ...
        dim1 = create(
            self._fileobject, "IfcDimensionalExponents", [0, 0, 0, 0, 0, 0, 0]
        )
        dim2 = create(
            self._fileobject, "IfcSIUnit", [dim1, "LENGTHUNIT", "MILLI", "METRE"]
        )
        dim3 = create(
            self._fileobject, "IfcSIUnit", [dim1, "AREAUNIT", None, "SQUARE_METRE"]
        )
        dim4 = create(
            self._fileobject, "IfcSIUnit", [dim1, "VOLUMEUNIT", None, "CUBIC_METRE"]
        )
        dim6 = create(
            self._fileobject, "IfcSIUnit", [dim1, "PLANEANGLEUNIT", None, "RADIAN"]
        )
        ...
        dim9 = create(
            ...
            [dim1, "PLANEANGLEUNIT", "DEGREE", dim8],
        )
        units = create(
            self._fileobject, "IfcUnitAssignment", [[dim2, dim3, dim4, dim9]]
        )
        self.Project = create(
            self._fileobject,
            "IfcProject",
    ...
    def __repr__(self):
        return (
            "IFC document " + self.Name
        )  # + " containing " + str(len(holder)) + " entities"
    ...
    def addStorey(
        self, building=None, placement=None, name="Default storey", description=None
    ):
    ...
    def addGroup(self, entities, name="Default group", description=None):
        ...
        gro = create(
            self._fileobject,
            "IfcGroup",
            [uid(), self._owner, str(name), description, None],
        )
        rel = create(
            self._fileobject,
    ...
    def addProduct(
        ...
        prd = create(
            self._fileobject, "IfcProductDefinitionShape", [None, None, representations]
        )
        try:
            elt = create(
                self._fileobject,
                elttype,
                [uid(), self._owner, name, description, None, placement, prd, None]
                + extra,
            )
        except Exception:
            print(
                ...
                elttype,
                " with attributes: ",
                [uid(), self._owner, str(name), description, None, placement, prd, None]
                + extra,
            )
            try:
                if hasattr(ifcw, "Entity"):
            ...
            for curve in data:
                ...
                if curve[0] == "line":
                    ...
                    if curve[5] == "CARTESIAN":
                        ...
                        trim1 = create(
                            None, "IfcCartesianPoint", getTuple(curve[3][0])[:2]
                        )
                        trim2 = create(
                            None, "IfcCartesianPoint", getTuple(curve[3][1])[:2]
                        )
                    else:
    ...
    def addExtrusion(self, profile, extrusion, placement=None):
        ...
        solid = create(
            self._fileobject, "IfcExtrudedAreaSolid", [profile, placement, edir, value]
        )
        return solid
