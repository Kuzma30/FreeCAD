...
def read(filename):
    ...
    if not handler.makeIndividualWalls:
        ...
        for k, lines in handler.lines.items():
            sk = FreeCAD.ActiveDocument.addObject(
                "Sketcher::SketchObject", "Walls_trace"
            )
            for l in lines:
            ...
            k = k.split(";")
            walls.append(
                Arch.makeWall(baseobj=sk, width=float(k[0]), height=float(k[1]))
            )
        for d in delete:
...
class SH3DHandler(xml.sax.ContentHandler):
    ...
    def startElement(self, tag, attributes):
        ...
        if tag == "wall":
            name = attributes["id"]
            p1 = FreeCAD.Vector(
                float(attributes["xStart"]) * 10, float(attributes["yStart"]) * 10, 0
            )
            p2 = FreeCAD.Vector(
                float(attributes["xEnd"]) * 10, float(attributes["yEnd"]) * 10, 0
            )
            height = float(attributes["height"]) * 10
            thickness = float(attributes["thickness"]) * 10
            if DEBUG:
                print("Creating wall: ", name)
            line = Draft.makeLine(p1, p2)
            if self.makeIndividualWalls:
                wall = Arch.makeWall(
                    baseobj=line, width=thickness, height=height, name=name
                )
                wall.Label = name
            else:
                self.lines.setdefault(str(thickness) + ";" + str(height), []).append(
                    line
                )
            ...
            p = p.add(
                FreeCAD.Vector(
                    float(attributes["x"]) * 10, float(attributes["y"]) * 10, 0
                )
            )
            ...
            if shape:
                ...
                p = p.add(
                    FreeCAD.Vector(0, 0, shape.BoundBox.Center.z - shape.BoundBox.ZMin)
                )
