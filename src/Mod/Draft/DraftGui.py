...
inCommandShortcuts = {
    ...
    "Global": [
        Draft.getParam("inCommandShortcutGlobal", "G"),
...
# UNITS handling
# ---------------------------------------------------------------------------
from draftutils.units import getDefaultUnit, makeFormatSpec, displayExternal
...
class DraftToolBar:
    ...
    def _pushbutton(
        ...
        if icon:
            if icon.endswith(".svg"):
                ...
                button.setIcon(
                    QtGui.QIcon.fromTheme(icon, QtGui.QIcon(":/icons/" + icon + ".svg"))
                )
    ...
    def _spinbox(
        self, name, layout, val=None, vmax=None, hide=True, double=False, size=None
    ):
        if double:
    ...
    def setupToolBar(self, task=False):
        ...
        self.angleValue.setText(
            FreeCAD.Units.Quantity(0, FreeCAD.Units.Angle).UserString
        )
        ...
        self.radiusValue.setText(
            FreeCAD.Units.Quantity(0, FreeCAD.Units.Length).UserString
        )
        self.isRelative = self._checkbox(
            "isRelative", self.layout, checked=self.relativeMode
        )
        ...
        self.continueCmd = self._checkbox(
            "continueCmd", self.layout, checked=self.continueMode
        )
        ...
        bl = QtGui.QHBoxLayout()
        self.layout.addLayout(bl)
        self.orientWPButton = self._pushbutton(
            "orientWPButton", bl, icon="Draft_SelectPlane"
        )
        bl = QtGui.QHBoxLayout()
        self.layout.addLayout(bl)
        ...
        self.isSubelementMode = self._checkbox(
            "isSubelementMode", self.layout, checked=False
        )
        ...
        QtCore.QObject.connect(
            self.xValue, QtCore.SIGNAL("valueChanged(double)"), self.changeXValue
        )
        QtCore.QObject.connect(
            self.yValue, QtCore.SIGNAL("valueChanged(double)"), self.changeYValue
        )
        QtCore.QObject.connect(
            self.zValue, QtCore.SIGNAL("valueChanged(double)"), self.changeZValue
        )
        QtCore.QObject.connect(
            self.lengthValue,
            QtCore.SIGNAL("valueChanged(double)"),
            self.changeLengthValue,
        )
        QtCore.QObject.connect(
            self.angleValue,
            QtCore.SIGNAL("valueChanged(double)"),
            self.changeAngleValue,
        )
        QtCore.QObject.connect(
            self.angleLock, QtCore.SIGNAL("stateChanged(int)"), self.toggleAngle
        )
        QtCore.QObject.connect(
            self.radiusValue,
            QtCore.SIGNAL("valueChanged(double)"),
            self.changeRadiusValue,
        )
        QtCore.QObject.connect(
            self.xValue, QtCore.SIGNAL("returnPressed()"), self.checkx
        )
        QtCore.QObject.connect(
            self.yValue, QtCore.SIGNAL("returnPressed()"), self.checky
        )
        QtCore.QObject.connect(
            self.lengthValue, QtCore.SIGNAL("returnPressed()"), self.checklength
        )
        QtCore.QObject.connect(
            self.xValue, QtCore.SIGNAL("textEdited(QString)"), self.checkSpecialChars
        )
        QtCore.QObject.connect(
            self.yValue, QtCore.SIGNAL("textEdited(QString)"), self.checkSpecialChars
        )
        QtCore.QObject.connect(
            self.zValue, QtCore.SIGNAL("textEdited(QString)"), self.checkSpecialChars
        )
        QtCore.QObject.connect(
            self.lengthValue,
            QtCore.SIGNAL("textEdited(QString)"),
            self.checkSpecialChars,
        )
        QtCore.QObject.connect(
            self.radiusValue,
            QtCore.SIGNAL("textEdited(QString)"),
        ...
        QtCore.QObject.connect(
            self.zValue, QtCore.SIGNAL("returnPressed()"), self.validatePoint
        )
        QtCore.QObject.connect(
            self.pointButton, QtCore.SIGNAL("clicked()"), self.validatePoint
        )
        QtCore.QObject.connect(
            self.radiusValue, QtCore.SIGNAL("returnPressed()"), self.validatePoint
        )
        QtCore.QObject.connect(
            self.angleValue, QtCore.SIGNAL("returnPressed()"), self.validatePoint
        )
        QtCore.QObject.connect(
            self.textValue, QtCore.SIGNAL("textChanged()"), self.checkEnterText
        )
        QtCore.QObject.connect(
            self.textOkButton, QtCore.SIGNAL("clicked()"), self.sendText
        )
        QtCore.QObject.connect(
            self.zValue, QtCore.SIGNAL("returnPressed()"), self.setFocus
        )
        ...
        QtCore.QObject.connect(
            self.finishButton, QtCore.SIGNAL("pressed()"), self.finish
        )
        QtCore.QObject.connect(
            self.closeButton, QtCore.SIGNAL("pressed()"), self.closeLine
        )
        QtCore.QObject.connect(
            self.wipeButton, QtCore.SIGNAL("pressed()"), self.wipeLine
        )
        QtCore.QObject.connect(
            self.orientWPButton, QtCore.SIGNAL("pressed()"), self.orientWP
        )
        QtCore.QObject.connect(
            self.undoButton, QtCore.SIGNAL("pressed()"), self.undoSegment
        )
        QtCore.QObject.connect(
            self.selectButton, QtCore.SIGNAL("pressed()"), self.selectEdge
        )
        QtCore.QObject.connect(
            self.continueCmd, QtCore.SIGNAL("stateChanged(int)"), self.setContinue
        )

        QtCore.QObject.connect(
            self.isCopy, QtCore.SIGNAL("stateChanged(int)"), self.setCopymode
        )
        QtCore.QObject.connect(
            self.isSubelementMode,
            QtCore.SIGNAL("stateChanged(int)"),
            self.setSubelementMode,
        )

        QtCore.QObject.connect(
            self.isRelative, QtCore.SIGNAL("stateChanged(int)"), self.setRelative
        )
        QtCore.QObject.connect(
            self.isGlobal, QtCore.SIGNAL("stateChanged(int)"), self.setGlobal
        )
        QtCore.QObject.connect(
            self.hasFill, QtCore.SIGNAL("stateChanged(int)"), self.setFill
        )
        QtCore.QObject.connect(
            self.baseWidget, QtCore.SIGNAL("resized()"), self.relocate
        )
        QtCore.QObject.connect(
            self.baseWidget, QtCore.SIGNAL("retranslate()"), self.retranslateUi
        )
        QtCore.QObject.connect(
            self.SSizeValue,
            QtCore.SIGNAL("valueChanged(double)"),
            self.changeSSizeValue,
        )
        QtCore.QObject.connect(
            self.SSizeValue, QtCore.SIGNAL("returnPressed()"), self.validateSNumeric
        )
        QtCore.QObject.connect(
            self.STrackValue,
            QtCore.SIGNAL("valueChanged(double)"),
            self.changeSTrackValue,
        )
        QtCore.QObject.connect(
            self.STrackValue, QtCore.SIGNAL("returnPressed()"), self.validateSNumeric
        )
        QtCore.QObject.connect(
            self.SStringValue, QtCore.SIGNAL("returnPressed()"), self.validateSString
        )
        QtCore.QObject.connect(
            self.chooserButton, QtCore.SIGNAL("pressed()"), self.pickFile
        )
        QtCore.QObject.connect(
            self.FFileValue, QtCore.SIGNAL("returnPressed()"), self.validateFile
        )
    ...
    def setupTray(self):
        ...
        QtCore.QObject.connect(
            self.wplabel, QtCore.SIGNAL("pressed()"), self.selectplane
        )
        QtCore.QObject.connect(
            self.styleButton, QtCore.SIGNAL("pressed()"), self.setstyle
        )
        QtCore.QObject.connect(
            self.constrButton, QtCore.SIGNAL("toggled(bool)"), self.toggleConstrMode
        )
        QtCore.QObject.connect(
            self.autoGroupButton, QtCore.SIGNAL("pressed()"), self.runAutoGroup
        )
    ...
    def retranslateUi(self, widget=None):
        ...
        self.pointButton.setToolTip(
            translate("draft", "Enter a new point with the given coordinates")
        )
        ...
        self.angleLock.setToolTip(
            translate("draft", "Check this to lock the current angle")
            + " ("
        ...
        self.isGlobal.setText(
            translate("draft", "Global") + " (" + inCommandShortcuts["Global"][0] + ")"
        )
        ...
        self.hasFill.setText(
            translate("draft", "Filled") + " (" + inCommandShortcuts["Fill"][0] + ")"
        )
        self.hasFill.setToolTip(
            translate(
                "draft",
                "Check this if the object should appear as filled, "
                + "otherwise it will appear as wireframe.\nNot available if "
                + "Draft preference option 'Use Part Primitives' is enabled",
            )
        )
        self.finishButton.setText(
            translate("draft", "Finish") + " (" + inCommandShortcuts["Exit"][0] + ")"
        )
        ...
        self.continueCmd.setToolTip(
            translate(
                "draft",
                "If checked, command will not finish until you press "
        ...
        self.closeButton.setText(
            translate("draft", "Close") + " (" + inCommandShortcuts["Close"][0] + ")"
        )
        self.closeButton.setToolTip(
            translate("draft", "Finishes and closes the current line")
        )
        self.wipeButton.setText(
            translate("draft", "Wipe") + " (" + inCommandShortcuts["Wipe"][0] + ")"
        )
        ...
        self.orientWPButton.setText(
            translate("draft", "Set WP") + " (" + inCommandShortcuts["SetWP"][0] + ")"
        )
        ...
        self.selectButton.setText(
            translate("draft", "Select edge")
            + " ("
            + inCommandShortcuts["SelectEdge"][0]
            + ")"
        )
        self.selectButton.setToolTip(
            translate(
                "draft", "Selects an existing edge to be measured by this dimension"
            )
        )
        ...
        self.isCopy.setText(
            translate("draft", "Copy") + " (" + inCommandShortcuts["Copy"][0] + ")"
        )
        self.isCopy.setToolTip(
            translate(
                "draft",
                "If checked, objects will be copied instead of moved. Preferences -> Draft -> Global copy mode to keep this mode in next commands",
            )
        )
        self.isSubelementMode.setText(
            translate("draft", "Modify subelements")
            + " ("
            + inCommandShortcuts["SubelementMode"][0]
            + ")"
        )
        ...
        self.chooserButton.setToolTip(
            translate("draft", "Open a FileChooser for font file")
        )
        self.textOkButton.setText(translate("draft", "Create text"))
        self.textOkButton.setToolTip(
            translate(
                "draft",
                "Press this button to create the text object, or finish your text with two blank lines",
            )
        )
    ...
    def retranslateTray(self, widget=None):
        ...
        self.wplabel.setToolTip(
            translate("draft", "Current working plane") + ":" + self.wplabel.text()
        )
        self.styleButton.setToolTip(
            translate("draft", "Change default style for new objects")
        )
    ...
    def setFocus(self, f=None):
        ...
        if p.GetBool("focusOnLength", False) and self.lengthValue.isVisible():
            ...
            self.lengthValue.setSelection(
                0, self.number_length(self.lengthValue.text())
            )
            ...
            self.radiusValue.setSelection(
                0, self.number_length(self.radiusValue.text())
            )
    ...
    def labelUi(self, title=translate("draft", "Label"), callback=None):
        ...
        l.addWidget(combo)
        QtCore.QObject.connect(
            combo, QtCore.SIGNAL("currentIndexChanged(int)"), callback
        )
    ...
    def getfacecol(self):
        ...
        if Draft.getParam("saveonexit", False):
            FreeCAD.ParamGet("User parameter:BaseApp/Preferences/View").SetUnsigned(
                "DefaultShapeColor", self.facecolor.rgb() << 8
            )
    ...
    def validatePoint(self):
        ...
        if self.sourceCmd or self.pointcallback:
            if self.labelRadius.isVisible():
                ...
                self.textValue.isVisible()
                or self.SStringValue.isVisible()
                or self.SSizeValue.isVisible()
                or self.STrackValue.isVisible()
            ...
            if self.labelSSize.isVisible():
                try:
                    ...
                    FreeCAD.Console.PrintMessage(
                        translate("draft", "Invalid Size value. Using 200.0.")
                    )
                ...
                try:
                    ...
                    FreeCAD.Console.PrintMessage(
                        translate("draft", "Invalid Tracking value. Using 0.")
                    )
        ...
        if self.sourceCmd:
            if self.labelSString.isVisible():
                if self.SStringValue.text():
                    ...
                    self.sourceCmd.validSString(
                        self.SStringValue.text()
                    )  # PySide returns Unicode from QString
                else:
                    FreeCAD.Console.PrintMessage(
                        translate("draft", "Please enter a text string.")
                    )
            ...
            if self.chooserButton.isVisible():
                try:
                    ...
                    if fname[0]:
                        ...
                        FreeCAD.Console.PrintMessage(
                            "DraftGui.pickFile: no file selected."
                        )  # can this happen?
        ...
        if self.sourceCmd:
            if self.labelFFile.isVisible():
                if self.FFileValue.text():
                    # QString to PyString
                    self.sourceCmd.validFFile(utf8_decode(self.FFileValue.text()))
                else:
                    FreeCAD.Console.PrintMessage(
    ...
    def popupMenu(self, llist, ilist=None, pos=None):
        ...
        QtCore.QObject.connect(
            self.groupmenu, QtCore.SIGNAL("triggered(QAction *)"), self.popupTriggered
        )
    ...
    def setAutoGroup(self, value=None):
        if value is None:
            ...
            if obj:
                ...
                self.autoGroupButton.setToolTip(
                    translate("draft", "Autogroup:") + " " + obj.Label
                )
                self.autoGroupButton.setDown(False)
            else:
                self.autogroup = None
...
class FacebinderTaskPanel:
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.title.setText(
            QtGui.QApplication.translate("draft", "Facebinder elements", None)
        )
