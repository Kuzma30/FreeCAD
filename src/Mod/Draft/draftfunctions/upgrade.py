...
def upgrade(objects, delete=False, force=None):
    ...
    if force:
        ...
        if groups:
            ...
            if result:
                _msg(
                    translate("draft", "Found groups: closing each open object inside")
                )
            ...
            # we have many separate faces: we try to make a shell or compound
                ...
                if result:
                    _msg(
                        translate(
                            "draft",
                            "Found several objects: creating a "
            ...
            # we have faces: we try to join them if they are coplanar
                ...
                if result:
                    _msg(
                        translate(
                            "draft",
                            "Found object with several coplanar faces: refine them",
                        )
                    )
            # only one object: if not parametric, we "draftify" it
            elif len(objects) == 1 and not objects[0].isDerivedFrom(
                ...
                if result:
                    _msg(
                        translate(
                            "draft", "Found 1 non-parametric objects: draftifying it"
                        )
                    )
        ...
        # in the following cases there are no faces
            ...
            if wires and not openwires and not loneedges:
                ...
                if len(objects) == 1 and objects[0].isDerivedFrom(
            ...
            # wires or edges: we try to join them
                ...
                if result:
                    if result == True:
                        _msg(
                            translate(
                                "draft", "Found several wires or edges: wiring them"
                            )
                        )
                    else:
                        _msg(
