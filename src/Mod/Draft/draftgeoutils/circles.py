...
def circlefrom1Line2Points(edge, p1, p2):
    ...
    cen1 = findIntersection(
        edg(projectedCen1, perpCen1), edg(mid, perp_mid), True, True
    )
    cen2 = findIntersection(
        edg(projectedCen2, perpCen2), edg(mid, perp_mid), True, True
    )
...
def circleFrom3LineTangents(edge1, edge2, edge3):
    ...
    def rot(ed):
        geo = Part.LineSegment(
            v1(ed), v1(ed).add(DraftVecUtils.rotate(vec(ed), math.pi / 2))
        )
        return geo.toShape()
