...
class Edit(gui_base_original.Modifier):
    ...
    def __init__(self):
        ...
        self.gui_tools_repository.add(
            "LinearDimension", edit_draft.DraftDimensionGuiTools()
        )
        ...
        self.gui_tools_repository.add(
            "Part::Cylinder", edit_part.PartCylinderGuiTools()
        )
    ...
    def register_selection_callback(self):
        ...
        self.unregister_selection_callback()
        self.selection_callback = self.view.addEventCallback(
    ...
    def mousePressed(self, event_callback):
        ...
        if (
            ...
            and event.getButton() == event.BUTTON1
        ):  # left click
    ...
    def mouseMoved(self, event_callback):
        ...
        if self.editing is not None:
            ...
            if ep is not None:
                ...
                self.trackers[str(node.objectName.getValue())][ep].setColor(
                    COLORS["red"]
                )
    ...
    def startEditing(self, obj, node_idx):
        ...
        App.Console.PrintMessage(
            obj.Name + ": editing node number " + str(node_idx) + "\n"
        )
    ...
    def updateTrackerAndGhost(self, event):
        ...
        snappedPos = Gui.Snapper.snap(
            (pos[0], pos[1]), self.node[-1], constrain=orthoConstrain
        )
    ...
    def resetTrackersBezier(self, obj):
        ...
        pointswithmarkers = [
            (obj.Shape.Edges[0].Curve.getPole(1), knotmarkers[firstknotcont])
        ]
        ...
        for index, pwm in enumerate(pointswithmarkers):
            ...
            self.trackers[obj.Name].append(
                trackers.editTracker(
                    p, obj.Name, index, obj.ViewObject.LineColor, marker=marker
                )
            )
    ...
    def updateGhost(self, obj, node_idx, v):
        ...
        if self.current_editing_object_gui_tools:
            self.current_editing_object_gui_tools.update_preview_object(
                self, obj, node_idx, v
            )
