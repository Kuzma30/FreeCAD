    ...
    def Activated(self):
        ...
        if FreeCADGui.Selection.getSelection():
            ...
            FreeCAD.Console.PrintError(
                translate(
                    "Draft", "You must choose a base object before using this command"
                )
                + "\n"
            )
...
class Draft_Hatch_TaskPanel:
    def __init__(self, baseobj):
        ...
        self.p1 = FreeCAD.ParamGet(
            "User parameter:BaseApp/Preferences/Mod/TechDraw/PAT"
        )
        ...
        if pat in [
            self.form.Pattern.itemText(i) for i in range(self.form.Pattern.count())
        ]:
    ...
    def accept(self):
        ...
        if hasattr(self.baseobj, "File") and hasattr(self.baseobj, "Pattern"):
            ...
            FreeCADGui.doCommand(
                o + '.File="' + self.form.File.property("fileName") + '"'
            )
            FreeCADGui.doCommand(
                o + '.Pattern="' + self.form.Pattern.currentText() + '"'
            )
