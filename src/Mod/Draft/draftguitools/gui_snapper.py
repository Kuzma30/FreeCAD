...
class Snapper:
    ...
    def init_active_snaps(self):
        ...
        snap_modes = param.GetString(
            "snapModes", "100000000000000"
        )  # default value: only lock is ON
        i = 0
    ...
    def snap(
        self, screenpos, lastpoint=None, active=True, constrain=False, noTracker=False
    ):
        ...
        if self.snapInfo and "Component" in self.snapInfo:
            osnap = self.snapToObject(
                lastpoint, active, constrain, eline, point, oldActive
            )
            if osnap:
                return osnap
    ...
    def snapToObject(self, lastpoint, active, constrain, eline, point, oldActive):
        ...
        if not active:
            ...
            shape = Part.getShape(
                parent, subname, needSubElement=True, noElementMap=True
            )
            ...
            if not shape.isNull():
                ...
                if Draft.getType(obj) == "Polygon":
                    ...
                    snaps.append(
                        [obj.Placement.Base, "center", self.toWP(obj.Placement.Base)]
                    )
    ...
    def snapToDim(self, obj):
        snaps = []
        if obj.ViewObject:
            if hasattr(obj.ViewObject.Proxy, "p2") and hasattr(
                obj.ViewObject.Proxy, "p3"
            ):
                snaps.append(
                    [
                        obj.ViewObject.Proxy.p2,
                        "endpoint",
                        self.toWP(obj.ViewObject.Proxy.p2),
                    ]
                )
                snaps.append(
                    [
                        obj.ViewObject.Proxy.p3,
                        "endpoint",
                        self.toWP(obj.ViewObject.Proxy.p3),
                    ]
                )
        return snaps
        ...
        if self.isEnabled("Extension"):
            ...
            if tsnap:
                if (tsnap[0].sub(point)).Length < self.radius:
                    ...
                    if self.extLine:
                        self.extLine.p2(tsnap[2])
                        self.extLine.color.rgb = Gui.draftToolBar.getDefaultColor(
                            "line"
                        )
                        self.extLine.on()
                    self.setCursor(tsnap[1])
                                ...
                                if np != e.Vertexes[0].Point:
                                    ...
                                    if self.extLine:
                                        if self.snapStyle:
                                            ...
                                            self.extLine.p1(p0)
                                        self.extLine.p2(np)
                                        self.extLine.color.rgb = (
                                            Gui.draftToolBar.getDefaultColor("line")
                                        )
                                        self.extLine.on()
                                    ...
                                    if len(self.lastExtensions) == 0:
                                        ...
                                        if not DraftGeomUtils.areColinear(
                                            ...
                                            self.lastExtensions.append(
                                                self.lastExtensions[0]
                                            )
                        ...
                        if (self.radius == 0) or (dv.Length <= self.radius):
                            ...
                            if self.extLine and self.extLine2:
                                if DraftVecUtils.equals(
                                    self.extLine.p1(),
                                    self.lastExtensions[0].Vertexes[0].Point,
                                ):
    ...
    def snapToPolar(self, point, last):
        ...
        if self.isEnabled("Ortho") and (not self.mask):
            if last:
                ...
                if hasattr(App, "DraftWorkingPlane"):
                    ax = [
                        App.DraftWorkingPlane.u,
                        App.DraftWorkingPlane.v,
                        App.DraftWorkingPlane.axis,
                    ]
                else:
                    ax = [App.Vector(1, 0, 0), App.Vector(0, 1, 0), App.Vector(0, 0, 1)]
                ...
                for v in vecs:
                    if not DraftVecUtils.isNull(v):
                        ...
                        np = self.getPerpendicular(de, point)
                        if (
                            (self.radius == 0) and (point.sub(last).getAngle(v) < 0.087)
                    ...
                    if last:
                        if DraftGeomUtils.geomType(shape) == "Line":
                            if self.constraintAxis:
                                ...
                                pt = DraftGeomUtils.findIntersection(
                                    tmpEdge, shape, True, True
                                )
                                if pt:
                                    for p in pt:
    ...
    def snapToExtOrtho(self, last, constrain, eline):
        ...
        if self.isEnabled("Extension") and self.isEnabled("Ortho"):
            if constrain and last and self.constraintAxis and self.extLine:
                tmpEdge1 = Part.LineSegment(
                    last, last.add(self.constraintAxis)
                ).toShape()
                tmpEdge2 = Part.LineSegment(
                    self.extLine.p1(), self.extLine.p2()
                ).toShape()
                # get the intersection points
    ...
    def snapToHold(self, point):
        ...
        if len(self.holdPoints) > 1:
            ...
            while len(l) > 1:
                p1 = l.pop()
                for p2 in l:
                    i1 = DraftGeomUtils.findIntersection(
                        p1, p1.add(u), p2, p2.add(v), True, True
                    )
                    if i1:
                        ipoints.append([p1, i1[0]])
                    i2 = DraftGeomUtils.findIntersection(
            ...
            for o in self.lastObj:
                ...
                if obj:
                    if obj.isDerivedFrom("Part::Feature") or (
                        Draft.getType(obj) == "Axis"
                    ):
                        if (not self.maxEdges) or (
                            len(obj.Shape.Edges) <= self.maxEdges
                        ):
                            import Part
                            ...
                            for e in obj.Shape.Edges:
                                ...
                                try:
                                    if (
                                        ...
                                        and hasattr(e, "Curve")
                                        and isinstance(
                                            e.Curve, (Part.Line, Part.LineSegment)
                                        )
                                        and hasattr(shape, "Curve")
                                        and isinstance(
                                            shape.Curve, (Part.Line, Part.LineSegment)
                                        )
                                    ):
            ...
            if mode != self.cursorMode:
                ...
                for w in mw.findChild(QtGui.QMdiArea).findChildren(QtGui.QWidget):
                    if (
                        ...
                        == "SIM::Coin3D::Quarter::QuarterWidget"
                    ):
                        w.setCursor(cur)
    ...
    def constrain(self, point, basepoint=None, axis=None):
        ...
        if not self.constrainLine:
            if self.snapStyle:
                self.constrainLine = trackers.lineTracker(
                    scolor=Gui.draftToolBar.getDefaultColor("snap")
                )
            else:
    ...
    def getPoint(
        ...
        def move(event_cb):
            ...
            self.pt = Gui.Snapper.snap(
                mousepos, lastpoint=last, active=ctrl, constrain=shift
            )
            if hasattr(App, "DraftWorkingPlane"):
                self.ui.displayPoint(
                    self.pt,
                    last,
        ...
        if callback:
            if title:
                ...
                interface(
                    cancel=cancel, getcoords=getcoords, extra=extradlg, rel=bool(last)
                )
    ...
    def init_draft_snap_buttons(self, commands, context, button_suffix):
        ...
        for gc in commands:
            ...
            QtCore.QObject.connect(
                ...
                lambda f=Gui.doCommand, arg=command: f(arg),
            )
    ...
    def restore_snap_buttons_state(self, toolbar, button_suffix):
        ...
        for action in toolbar.findChildren(QtGui.QAction):
            ...
            if snap in self.active_snaps:
                ...
                snap in Gui.Snapper.snaps
            ):  # required: the toolbar has more children than the buttons
                action.setChecked(False)
                action.setToolTip(
                    action.toolTip() + " " + (translate("draft", "(OFF)"))
                )
