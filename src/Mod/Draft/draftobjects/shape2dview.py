    ...
    def setProperties(self, obj):
        ...
        if not "Base" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property", "The base object this 2D view must represent"
            )
            obj.addProperty("App::PropertyLink", "Base", "Draft", _tip)
        if not "Projection" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property", "The projection vector of this object"
            )
            obj.addProperty("App::PropertyVector", "Projection", "Draft", _tip)
            obj.Projection = App.Vector(0, 0, 1)
        if not "ProjectionMode" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property", "The way the viewed object must be projected"
            )
            obj.addProperty("App::PropertyEnumeration", "ProjectionMode", "Draft", _tip)
            obj.ProjectionMode = [
                "Solid",
                "Individual Faces",
                "Cutlines",
        ...
        if not "FaceNumbers" in pl:
            ...
            obj.addProperty("App::PropertyIntegerList", "FaceNumbers", "Draft", _tip)
        if not "HiddenLines" in pl:
            _tip = QT_TRANSLATE_NOOP("App::Property", "Show hidden lines")
            obj.addProperty("App::PropertyBool", "HiddenLines", "Draft", _tip)
        ...
        if not "FuseArch" in pl:
            ...
            obj.addProperty("App::PropertyBool", "FuseArch", "Draft", _tip)
        if not "Tessellation" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property", "Tessellate Ellipses and B-splines into line segments"
            )
            obj.addProperty("App::PropertyBool", "Tessellation", "Draft", _tip)
        ...
        if not "InPlace" in pl:
            ...
            obj.addProperty("App::PropertyBool", "InPlace", "Draft", _tip)
            obj.InPlace = True
        if not "SegmentLength" in pl:
            ...
            obj.addProperty("App::PropertyFloat", "SegmentLength", "Draft", _tip)
        ...
        if not "VisibleOnly" in pl:
            ...
            obj.addProperty("App::PropertyBool", "VisibleOnly", "Draft", _tip)
        ...
        if not "ExclusionPoints" in pl:
            ...
            obj.addProperty("App::PropertyVectorList", "ExclusionPoints", "Draft", _tip)
        if not "OnlySolids" in pl:
            ...
            obj.addProperty("App::PropertyBool", "OnlySolids", "Draft", _tip)
        if not "Clip" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property",
            ...
            obj.addProperty("App::PropertyBool", "Clip", "Draft", _tip)
        if not "AutoUpdate" in pl:
            _tip = QT_TRANSLATE_NOOP(
                "App::Property", "This object will be recomputed only if this is True."
            )
            obj.addProperty("App::PropertyBool", "AutoUpdate", "Draft", _tip)
            obj.AutoUpdate = True
            ...
            for e in s.Edges:
                for v in verts:
                    try:
                        ...
                        print(
                            "FIXME: shape2dview: distance unavailable for edge",
                            e,
                            "in",
                            obj.Label,
                        )
        ...
        if obj.Base:
            if utils.get_type(obj.Base) in ["BuildingPart", "SectionPlane"]:
                ...
                if utils.get_type(obj.Base) == "SectionPlane":
                    ...
                    m = 1
                    if obj.Base.ViewObject and hasattr(
                        obj.Base.ViewObject, "CutMargin"
                    ):
                ...
                if objs:
                    ...
                    if obj.ProjectionMode in ["Solid", "Solid faces"]:
                        ...
                        for sh in shapes:
                            ...
                            faces = []
                            if (obj.ProjectionMode == "Cutfaces") and (
                                sh.ShapeType == "Solid"
                            ):
                ...
                if not DraftVecUtils.isNull(obj.Projection):
                    if obj.ProjectionMode == "Solid":
                        obj.Shape = self.getProjected(
                            obj, obj.Base.Shape, obj.Projection
                        )
                        ...
                        App.Console.PrintWarning(
                            obj.ProjectionMode + " mode not implemented\n"
                        )
