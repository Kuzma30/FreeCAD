    ...
    class WriteXDMFTaskPanel:
        ...
        def __init__(self, fem_mesh_obj, fileString):
            self.form = FreeCADGui.PySideUic.loadUi(
                os.path.join(
                    FreeCAD.getHomePath(), "Mod/Fem/Resources/ui/MeshGroupXDMFExport.ui"
                )
            )
            self.result_dict = {}
...
def export(objectslist, fileString, group_values_dict_nogui=None):
    ...
    if fileString != "":
        ...
        if fileExtension.lower() == ".xml":
            Console.PrintWarning("XML is not designed to save higher order elements.\n")
            ...
            if mesh_groups != ():
                ...
                if FreeCAD.GuiUp == 1:
                    ...
                    if group_values_dict_nogui is None:
                        group_values_dict_nogui = dict(
                            [(g, (1, 0)) for g in mesh_groups]
                        )
                    writeFenicsXDMF.write_fenics_mesh_xdmf(
