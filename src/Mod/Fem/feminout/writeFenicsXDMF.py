...
def write_fenics_mesh_codim_xdmf(
    ...
    (topology_type, nodes_per_element) = FreeCAD_to_Fenics_XDMF_dict[
        (name_topo, element_order)
    ]
...
def write_fenics_mesh_xdmf(
    fem_mesh_obj, outputfile, group_values_dict={}, encoding=ENCODING_ASCII
):
    """
    ...
    Console.PrintMessage(
        "Celltype in mesh -> {} and its Fenics dolfin name: {}\n".format(
            celltype_in_mesh, cellname_fenics
        )
    )
    ...
    nodes_dict = write_fenics_mesh_points_xdmf(
        fem_mesh_obj, base_geometry, encoding=encoding
    )
    ...
    for g in gmshgroups:
        ...
        mesh_function_geometry = ET.SubElement(
            mesh_function_grid, "Geometry", Reference="XML"
        )
        ...
        val_array = np.array(
            [
                elem_dict.get(e, elem_mark_default)
                for e in mesh_function_topology_description
            ]
        )
