...
def get_femelements_by_references(
    femmesh, femelement_table, references, femnodes_ele_table=None
):
...
def get_femnodes_by_refshape(femmesh, ref):
    ...
    for refelement in ref[1]:
        ...
        FreeCAD.Console.PrintMessage(
            ...
            "Object label: {2}, "
            "Element name: {3}\n".format(
                r.ShapeType, ref[0].Name, ref[0].Label, refelement
            )
        )
...
def get_bit_pattern_dict(femelement_table, femnodes_ele_table, node_set):
    ...
    FreeCAD.Console.PrintLog(
        "len bit_pattern_dict: " + str(len(bit_pattern_dict)) + "\n"
    )
...
def get_femelement_sets(
    femmesh, femelement_table, fem_objects, femnodes_ele_table=None
):
...
def get_femelement_direction1D_set(
    femmesh, femelement_table, beamrotation_objects, theshape=None
):
    """
...
def get_beam_main_axis_m(beam_direction, defined_angle):
    ...
    if vector_a[0] != 0 and vector_a[1] == 0 and vector_a[2] == 0:
        ...
        dot_x = (
            vector_a[0] * normal_n[0]
            + vector_a[1] * normal_n[1]
            + vector_a[2] * normal_n[2]
        )
...
def get_force_obj_edge_nodeload_table(
    femmesh, femelement_table, femnodes_mesh, frc_obj
):
    # force_obj_node_load_table:
    #     [
    #         ("refshape_name.elemname", node_load_table),
    ...
    for o, elem_tup in frc_obj.References:
        for elem in elem_tup:
            ...
            for node in node_sum_length_table:
                ...
                node_load_table[node] = (
                    node_sum_length_table[node] * force_per_sum_ref_edge_length
                )
...
def get_ref_edgenodes_table(femmesh, femelement_table, refedge):
    ...
    if is_solid_femmesh(femmesh):
        ...
        refedge_fem_edgeelements = get_femelements_by_femnodes_std(
            femelement_table, refedge_nodes
        )
    ...
    for o, elem_tup in frc_obj.References:
        for elem in elem_tup:
            ...
            for node in node_sum_area_table:
                ...
                node_load_table[node] = (
                    node_sum_area_table[node] * force_per_sum_ref_face_area
                )
...
def use_correct_fluidinout_ele_def(
    FluidInletoutlet_ele, fileName, fluid_inout_nodes_file
):
    f = open(fileName, "r")
    cnt = 0
    line = f.readline()
