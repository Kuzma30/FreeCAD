...
class TestCcxTools(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # more inits
        self.mesh_name = "Mesh"
        self.test_file_dir = join(testtools.get_fem_test_home_dir(), "calculix")
    ...
    # ********************************************************************************************
    def test_thermomech_spine(self):
        # set up
...
def create_test_results():
    ...
    res_obj_thermo = doc_thermomech.getObject(
        "CCX_Results001"
    )  # two time step results after run
    for s in stat_types:
    ...
    thermomech_expected_values_file = join(
        thermomech_analysis_dir, "spine_thermomech_expected_values"
    )
    ...
    frd_result_file = os.path.splitext(fea.inp_file_name)[0] + ".frd"
    dat_result_file = os.path.splitext(fea.inp_file_name)[0] + ".dat"
    frd_thermomech_test_result_file = join(
        thermomech_analysis_dir, "spine_thermomech.frd"
    )
    dat_thermomech_test_result_file = join(
        thermomech_analysis_dir, "spine_thermomech.dat"
    )
