    ...
    class Ui_Dialog(object):
        ...
        def accept(self):
            ...
            try:
                ...
                self.label1.setText(
                    translate(
                        "Dialog", "<font color='red'>Enter distance</font>", None
                    )
                )
                return
                ...
                self.label1.setText(
                    translate(
                        "Dialog", "<font color='red'>Select ImagePlane</font>", None
                    )
                )
                return
        ...
        def reject(self):
            if len(self.stack) != 2:
                self.view.removeEventCallbackPivy(
                    pvy.SoMouseButtonEvent.getClassTypeId(), self.callback
                )
                self.view.removeEventCallbackPivy(
                    pvy.SoLocation2Event.getClassTypeId(), self.callmouse
                )
            self.stack = []
            self.tracker.off()
            self.tracker.finalize()
        ...
        def getpoint(self, event_cb):
            ...
            if event.getState() == pvy.SoMouseButtonEvent.DOWN:
                ...
                if len(self.stack) == 1:
                    ...
                    self.label1.setText(
                        translate(
                            "Dialog", "Select Image Plane and type distance", None
                        )
                    )
