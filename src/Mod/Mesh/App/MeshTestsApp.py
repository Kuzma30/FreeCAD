...
class MeshTopoTestCases(unittest.TestCase):
    ...
    def testCorruptedFacet(self):
        ...
        mesh = Mesh.Mesh()
        mesh.addFacet(
            v(1.0e1, -1.0e1, 1.0e1), v(1.0e1, +1.0e1, 1.0e1), v(0.0e0, 0.0e0, 1.0e1)
        )

        mesh.addFacet(
            v(-1.0e1, -1.0e1, 1.0e1), v(-1.0e1, +1.0e1, 1.0e1), v(0e0, 0.0e0, 1.0e1)
        )
        ...
        mesh.addFacet(
            v(-1.0e1, +1.0e1, 1.0e1), v(+1.0e1, +1.0e1, 1.0e1), v(+1.0e1, +1.0e1, 1.0e1)
        )

        mesh.addFacet(
            v(+1.0e1, +1.0e1, 1.0e1), v(+1.0e1, 00.0e1, 1.0e1), v(+1.0e1, -1.0e1, 1.0e1)
        )
...
class MeshSplitTestCases(unittest.TestCase):
    ...
    def testFindNearest(self):
        ...
        self.assertEqual(
            len(self.mesh.nearestFacetOnRay((0.5, 0.5, 0.5), (0, 0, 1))), 1
        )
        self.assertEqual(
            len(self.mesh.nearestFacetOnRay((0.5, 0.5, 0.5), (0, 0, 1), -math.pi / 2)),
            0,
        )
        self.assertEqual(
...
class MeshGeoTestCases(unittest.TestCase):
    ...
    def testIntersectionOnEdge(self):
        ...
        self.planarMesh.append(
            [4.6488823890686035, -1.7827962636947632, 0.4577442705631256]
        )
        self.planarMesh.append(
            [4.524135112762451, -2.0620131492614746, 0.5294350385665894]
        )
        self.planarMesh.append(
            [4.6488823890686035, -1.8261089324951172, 0.23069120943546295]
        )
        planarMeshObject = Mesh.Mesh(self.planarMesh)
        f1 = planarMeshObject.Facets[0]
        f2 = planarMeshObject.Facets[1]
...
class PolynomialFitCases(unittest.TestCase):
    ...
    def testFitBad(self):
        ...
        for i in d["Residuals"]:
            self.assertFalse(
                math.fabs(i) < 0.0001, "Residual %f must be higher" % math.fabs(i)
            )
