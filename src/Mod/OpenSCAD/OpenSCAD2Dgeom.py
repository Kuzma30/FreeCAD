...
class Overlappingfaces:
    ...
    def __init__(self, facelist):
        self.sortedfaces = sorted(
            facelist, key=(lambda shape: shape.Area), reverse=True
        )
        self.builddepdict()
        # self.faceindex = {}
    ...
    def builddepdict(self):
        ...
        for bigfacei, smallfacei in itertools.combinations(
            range(len(self.sortedfaces)), 2
        ):
            try:
    ...
    def printtree(isinsidedict, facenum):
        ...
        rootitems = [
            ...
            for fi in range(facenum)
            if Overlappingfaces.hasnoparentstatic(isinsidedict, fi)
        ]
        ...
        def addfeature(faceindex, isinsidedict):
            ...
            if len(directchildren) == 0:
                ...
                if len(directchildren) == 1:
                    ...
                    obj.Tool.Shapes = [
                        addfeature(child, subdict) for child in directchildren
                    ]
    ...
    def makeshape(self):
        ...
        while not all(
            [
                Overlappingfaces.hasnoparentstatic(isinsidedict, fi)
...
def endpointdistancedebuglist(debuglist):
    ...
    if numedges > 1:
        innerdistances = [
            ...
            .Point.sub(
                debuglist[i + 1][0].Vertexes[(not debuglist[i + 1][1]) * -1].Point
            )
            .Length
            for i in range(numedges - 1)
        ]
...
def subtractfaces2(faces):
    ...
    return fusefaces(
        [subtractfaces(facegroup) for facegroup in findoverlappingfaces(faces)]
    )
