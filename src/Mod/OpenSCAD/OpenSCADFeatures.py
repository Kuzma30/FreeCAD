...
class Resize:
    def __init__(self, obj, target, vector):
        ...
        obj.addProperty(
            "Part::PropertyPartShape", "Shape", "Resize", "Shape of the Resize"
        )
        obj.addProperty(
            "App::PropertyVector", "Vector", "Resize", " Resize Vector"
...
class MatrixTransform:
    def __init__(self, obj, matrix=None, child=None):
        ...
        obj.addProperty(
            "App::PropertyMatrix", "Matrix", "Matrix", "Transformation Matrix"
        )
        obj.Proxy = self
        obj.Matrix = matrix
        obj.Base = child
...
class RefineShape:
    ...
    def __init__(self, obj, child=None):
        obj.addProperty(
            "App::PropertyLink", "Base", "Base", "The base object that must be refined"
        )
        obj.Proxy = self
        obj.Base = child
...
class IncreaseTolerance:
    ...
    def __init__(self, obj, child, tolerance=0):
        ...
        obj.addProperty(
            "App::PropertyDistance", "Edge", "Tolerance", "Edges tolerance (0 default)"
        )
        obj.addProperty(
            "App::PropertyDistance", "Face", "Tolerance", "Faces tolerance (0 default)"
        )
        obj.Base = child
...
class Frustum:
    def __init__(self, obj, r1=1, r2=2, n=3, h=4):
        obj.addProperty(
            "App::PropertyInteger", "FacesNumber", "Base", "Number of faces"
        )
        obj.addProperty(
            "App::PropertyDistance",
            "Radius1",
        ...
        obj.addProperty(
            "App::PropertyDistance", "Height", "Base", "Height of the Frustum"
        )
        ...
        if all((fp.Radius1, fp.Radius2, fp.FacesNumber, fp.Height)):
            ...
            for ir, r in enumerate((fp.Radius1, fp.Radius2)):
                ...
                for i in range(fp.FacesNumber - 1):
                    ...
                    pts.append(
                        FreeCAD.Vector(
                            r.Value * math.cos(ang),
                            r.Value * math.sin(ang),
                            ir * fp.Height.Value,
                        )
                    )
                pts.append(pts[0])
...
class Twist:
    def __init__(self, obj, child=None, h=1.0, angle=0.0, scale=[1.0, 1.0]):
        ...
        obj.addProperty(
            "App::PropertyDistance", "Height", "Base", "Height of the Extrusion"
        )
        obj.addProperty(
        ...
        if fp.Base and fp.Height and fp.Base.Shape.isValid():
            ...
            for lower_face in fp.Base.Shape.Faces:
                ...
                if fp.Angle.Value == 0.0:
                    ...
                    auxiliary_spine = Part.makeHelix(
                        pitch, height, radius, 0.0, left_handed
                    )
...
class PrismaticToroid:
    def __init__(self, obj, child=None, angle=360.0, n=3):
        obj.addProperty(
            "App::PropertyLink", "Base", "Base", "The 2D face that will be swept"
        )
    ...
    def createGeometry(self, fp):
        ...
        if fp.Base and fp.Angle and fp.Segments and fp.Base.Shape.isValid():
            solids = []
            min_sweep_angle_per_segment = (
                360.0 / fp.Segments
            )  # This is how OpenSCAD defines $fn
...
def makeSurfaceVolume(filename):
    ...
    for col in range(num_cols - 1):
        ...
        lines.append(Part.makeLine(a, b))
    corner2 = FreeCAD.Vector(
        coords[last_row][last_col].x, coords[last_row][last_col].y, min_z - 1
    )
