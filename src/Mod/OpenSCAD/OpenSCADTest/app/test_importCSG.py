...
class TestImportCSG(unittest.TestCase):
    ...
    def setUp(self):
        self.test_dir = join(
            FreeCAD.getHomePath(), "Mod", "OpenSCAD", "OpenSCADTest", "data"
        )
...
""",
        ...
        polyhedron = (
            doc.ActiveObject
        )  # With paths, the polygon gets created as a wire...
    ...
    def test_import_union(self):
        doc = self.utility_create_scad(
            "union() { cube(15, center=true); sphere(10); }", "union"
        )
        object = doc.ActiveObject
        self.assertTrue(object is not None)
    ...
    def test_import_linear_extrude(self):
        doc = self.utility_create_scad(
            "linear_extrude(height = 20) square([20, 10], center = true);",
            "linear_extrude_simple",
        )
        object = doc.ActiveObject
        self.assertTrue(object is not None)
    ...
    def test_import_resize(self):
        ...
        doc = self.utility_create_scad(
            "resize([2,0,0], auto=true) cube();", "resize_with_auto"
        )
        object = doc.ActiveObject
        self.assertTrue(object is not None)
        self.assertAlmostEqual(object.Shape.Volume, 8.000000, 6)
        ...
        doc = self.utility_create_scad(
            "resize([2,2,2]) cube([2,2,2]);", "resize_no_change"
        )
        object = doc.ActiveObject
        self.assertTrue(object is not None)
        self.assertAlmostEqual(object.Shape.Volume, 8.000000, 6)
        ...
        doc = self.utility_create_scad(
            "resize([2,2,2]) cube([4,8,12]);", "resize_non_uniform"
        )
        object = doc.ActiveObject
        self.assertTrue(object is not None)
        self.assertAlmostEqual(object.Shape.Volume, 8.000000, 6)
...
resize(newsize = [0,0,10], auto = [0,0,0]) {
    sphere($fn = 96, $fa = 12, $fs = 2, r = 8.5);
        ...
        object = doc.ActiveObject
        self.assertTrue(object is not None)
        object.Shape.tessellate(
            0.025
        )  # To ensure the bounding box calculation is correct
    ...
    def test_import_surface(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            testfile = join(self.test_dir, "Surface.dat").replace("\\", "/")
            doc = self.utility_create_scad(
                f'surface(file = "{testfile}", convexity = 5);',
                "surface_uncentered_dat",
            )
            object = doc.ActiveObject
            self.assertTrue(object is not None)
            self.assertAlmostEqual(object.Shape.Volume, 275.000000, 6)
