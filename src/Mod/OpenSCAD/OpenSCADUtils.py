...
def searchforopenscadexe():
    ...
    if sys.platform == "win32":
        ...
        if "ProgramW6432" in os.environ:
            testpaths.append(
                os.path.join(
                    os.environ.get("ProgramW6432", "C:"), "OpenSCAD\\openscad.exe"
                )
            )
...
def workaroundforissue128needed():
    ...
    return year < 2012 or (
        year == 2012
        and (mon < 6 or (mon == 6 and (len(vdate) < 3 or int(vdate[2]) <= 23)))
    )
...
def callopenscad(inputfilename, outputfilename=None, outputext="csg", keepname=False):
    ...
    if transferMechanism == 0:  # Use the Python temp-directory creation function
        ...
        return call_openscad_with_pipes(
            inputfilename, outputfilename, outputext, keepname
        )
    else:
        ...
        if not outputfilename:
            ...
            if keepname:
                outputfilename = os.path.join(
                    dir1,
                    "%s.%s"
                    % (os.path.split(inputfilename)[1].rsplit(".", 1)[0], outputext),
                )
            else:
                outputfilename = os.path.join(
                    dir1, "%s.%s" % (next(tempfilenamegen), outputext)
                )
...
def call_openscad_with_pipes(
    input_filename, output_filename, output_extension, keep_name
):
    ...
    with open(input_filename) as datafile:
        ...
        p = subprocess.Popen(
            [openscad_executable, "--export-format", "csg", "-o", "-", "-"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        ...
        if not output_filename:
            ...
            if keep_name:
                output_filename = os.path.join(
                    ...
                    "%s.%s"
                    % (
                        os.path.split(input_filename)[1].rsplit(".", 1)[0],
                        output_extension,
                    ),
                )
            else:
...
def isspecialorthogonalpython(submat, precision=4):
    return (
        isorthogonal(submat, precision) and round(detsubmatrix(submat), precision) == 1
    )
...
def meshopinline(opname, iterable1):
    ...
    return callopenscadmeshstring(
        "%s(){%s}"
        % (opname, " ".join((mesh2polyhedron(meshobj) for meshobj in iterable1)))
    )
...
def meshoptempfile(opname, iterable1):
    ...
    meshimports = " ".join(
        'import(file = "%s");' % os.path.split(filename)[1]  # filename \
    ...
    for obj in inobjs:
        if obj.isDerivedFrom("Mesh::Feature"):
            ...
            if False:  # disabled due to issue 1292
                ...
                meshes.append(
                    Mesh.Mesh(
                        obj.Shape.tessellate(params.GetFloat("meshmaxlength", 1.0))
                    )
                )
        else:
...
def process_ObjectsViaOpenSCAD(doc, children, name):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) for obj in children):
        ...
        FreeCAD.Console.PrintError(
            translate(
                "OpenSCAD", "Error all shapes must be either 2D or both must be 3D"
            )
            + u"\n"
        )
