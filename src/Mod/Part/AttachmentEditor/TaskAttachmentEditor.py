...
def LinkFromStr(strlink, document):
    ...
    subname = ''
    if feature is None:
        raise ValueError(translate('AttachmentEditor',"No object named {name}",None).format(name= pieces[0]))
    if len(pieces) == 2:
        subname = pieces[1]
    elif len(pieces) > 2:
    ...
    def __init__(self, obj_to_attach,
        ...
        if hasattr(obj_to_attach,'Attacher'):
            ...
            if not movable:
                ...
                raise ValueError(translate('AttachmentEditor',"Object {name} is neither movable nor attachable, can't edit attachment",None)
        ...
        if self.create_transaction:
            self.obj.Document.openTransaction(translate('AttachmentEditor',"Edit attachment of {feat}",None).format(feat= self.obj.Name))
    ...
    def addSelection(self,docname,objname,subname,pnt):
        ...
        if i > -1:
            ...
            if App.getDocument(docname).getObject(objname) in getAllDependent(self.obj):
                self.form.message.setText(translate('AttachmentEditor',"{obj1} depends on object being attached, can't use it for attachment",None).format(obj1= objname))
                return
        ...
        try:
            ...
            for m in sugr['reachableModes'].keys():
                ...
                if len(listlistrefs) == 1:
                    ...
                    txt = translate('AttachmentEditor',"{mode} (add {morerefs})",None).format(mode= txt,
                                                           morerefs= u"+".join(listrefs_userfriendly))
                else:
                    txt = translate('AttachmentEditor',"{mode} (add more references)",None).format(mode= txt)
                item.setText(txt)
                item.setData(self.KEYmode,m)
                item.setData(self.KEYon,True)
                if m == sugr['bestFitMode']:
    ...
    def updateRefButtons(self):
        try:
            ...
            for i in range(len(self.refButtons)):
                ...
                typ = translate('AttachmentEditor',"Reference{i}",None).format(i= str(i+1))
    ...
    def updatePreview(self):
        ...
        try:
            ...
            if self.last_sugr['message'] == 'LinkBroken':
                raise ValueError(translate('AttachmentEditor',"Failed to resolve links. {err}",None).format(err= self.last_sugr['error']))
            ...
            if new_plm is None:
                ...
                self.form.message.setText(    translate('AttachmentEditor',"Attached with mode {mode}",None)
            ...
            self.form.message.setText(translate('AttachmentEditor',"Error: {err}",None).format(err= str(err)))
