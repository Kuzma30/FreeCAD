...
def StrListFromRefs(references):
    ...
    return [
        StrFromLink(feature, subelement)
        for (feature, subelement) in references_oldstyle
    ]
...
class AttachmentEditorTaskPanel(FrozenClass):
    ...
    def __define_attributes(self):
        ...
        self.refLines = (
            []
        )  # reference lineEdit widgets, packed into a list for convenience
        ...
        self.create_transaction = (
            True  # if false, dialog doesn't mess with transactions.
        )
        ...
        try:
            ...
            for i in range(len(self.refButtons)):
                ...
                if self.last_sugr is not None:
                    ...
                    if i < len(typestr):
                        typ = self.attacher.getRefTypeInfo(typestr[i])[
                            "UserFriendlyName"
                        ]
                btn.setText(
