...
class FeatureBooleanFragments:
    ...
    def execute(self, selfobj):
        ...
        selfobj.Shape = SplitAPI.booleanFragments(
            shapes, selfobj.Mode, selfobj.Tolerance
        )
...
def cmdCreateBooleanFragmentsFeature(name, mode):
    ...
    FreeCADGui.doCommand(
        "j = BOPTools.SplitFeatures.makeBooleanFragments(name='{name}')".format(
            name=name
        )
    )
    FreeCADGui.doCommand(
    ...
    try:
        ...
        btnOK = mb.addButton(
            _translate("Part_SplitFeatures", "Continue", None),
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Part_SplitFeatures", "Boolean fragments"
            ),
            "Accel": "",
...
class FeatureSlice:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLinkList", "Tools", "Slice", "Objects that slice."
        )
        obj.addProperty(
            "App::PropertyEnumeration",
            "Mode",
...
def cmdCreateSliceFeature(name, mode, transaction=True):
    ...
    FreeCADGui.addModule("BOPTools.SplitFeatures")
    FreeCADGui.doCommand(
        "f = BOPTools.SplitFeatures.makeSlice(name='{name}')".format(name=name)
    )
    FreeCADGui.doCommand(
        "f.Base = {sel}[0]\n"
...
class CommandSlice:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Part_SplitFeatures", "Slice to compound"
            ),
            "Accel": "",
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Part_SplitFeatures",
...
def cmdCreateXORFeature(name):
    ...
    FreeCADGui.addModule("BOPTools.SplitFeatures")
    FreeCADGui.doCommand(
        "j = BOPTools.SplitFeatures.makeXOR(name='{name}')".format(name=name)
    )
    FreeCADGui.doCommand(
        "j.Objects = {sel}".format(
            sel="["
