...
class Hole:
    ...
    def __init__(self, feature):
        ...
        self.feature.addProperty(
            "App::PropertyString", "HoleType", "Hole", "Type of hole"
        ).HoleType = "Depth"
        self.feature.addProperty(
            "App::PropertyBool", "Threaded", "Hole", "Threaded hole"
        ).Threaded = False
        self.feature.addProperty(
            "App::PropertyBool", "Counterbore", "Hole", "Counterbore hole"
        ).Counterbore = False
        self.feature.addProperty(
            "App::PropertyBool", "Countersink", "Hole", "Countersink hole"
        ).Countersink = False
        self.feature.addProperty(
            "App::PropertyString", "Norm", "Hole", "Name of norm"
        ).Norm = "Custom"
        self.feature.addProperty(
            "App::PropertyString", "NormTolerance", "Hole", "Tolerance field of norm"
        ).NormTolerance = "medium"
        self.feature.addProperty(
            "App::PropertyLength", "NormDiameter", "Hole", "Nominal diameter of hole"
        ).NormDiameter = 4.0
        self.feature.addProperty(
            "App::PropertyString",
            "ExtraNorm",
            "Hole",
            "Norm of bolt or washer used in hole",
        ).ExtraNorm = "ISO 4762"
        self.feature.addProperty(
            "App::PropertyString", "NormThread", "Hole", "Norm of thread"
        ).NormThread = "DIN 13-1"
        self.feature.addProperty(
            "App::PropertyString",
            "NormThreadFinish",
            "Hole",
            "Norm defining thread finish length",
        ).NormThreadFinish = "DIN 76-2"
        self.feature.addProperty(
            "App::PropertyLength", "Diameter", "Hole", "Diameter of hole"
        ).Diameter = 5.0
        self.feature.addProperty(
            "App::PropertyLength", "Depth", "Hole", "Depth of hole"
        ).Depth = 8.0
        self.feature.addProperty(
            "App::PropertyLength",
            "CounterboreDiameter",
        ...
        ).CounterboreDiameter = 10.0
        self.feature.addProperty(
            "App::PropertyLength", "CounterboreDepth", "Hole", "Depth of counterbore"
        ).CounterboreDepth = 4.0
        self.feature.addProperty(
            "App::PropertyLength", "CountersinkAngle", "Hole", "Angle of countersink"
        ).CountersinkAngle = 45.0
        self.feature.addProperty(
            "App::PropertyLength", "ThreadLength", "Hole", "Length of thread"
        ).ThreadLength = 5.0
        self.feature.addProperty(
            "App::PropertyString", "PositionType", "Hole", "Type of position references"
        ).PositionType = "Linear"
        self.feature.addProperty(
            "App::PropertyLinkSub", "Support", "Hole", "Support of hole feature"
        ).Support = None
        self.feature.addProperty(
            "App::PropertyLink",
            "HoleGroove",
            "Hole",
            "Revolution feature creating the hole",
        ).HoleGroove = None
        # Create new HoleGroove feature
        body = FreeCADGui.activeView().getActiveObject("pdbody")
        self.sketchaxis = self.feature.Document.addObject(
            "PartDesign::Line", "HoleSketchAxis"
        )
        body.addFeature(self.sketchaxis)
        self.Gui.ActiveDocument.hide(self.sketchaxis.Name)
        self.sketchplane = self.feature.Document.addObject(
            "PartDesign::Plane", "HoleSketchPlane"
        )
        ...
        self.sketch = self.feature.Document.addObject(
            "Sketcher::SketchObject", "HoleSketch"
        )
        self.sketch.Support = (self.sketchplane, ["front"])
        body.addFeature(self.sketch)
        self.Gui.ActiveDocument.hide(self.sketch.Name)
        feature.HoleGroove = feature.Document.addObject(
            "PartDesign::Groove", "HoleGroove"
        )
    ...
    def onChanged(self, fp, prop):
        ...
        if (
            ...
            or prop == "Diameter"
            or prop == "Depth"
            or prop == "CounterboreDiameter"
            or prop == "CounterboreDepth"
    ...
    def executePositionChanged(self, fp):
            ...
            for e in wire.Edges:
                if type(e.Curve) == Part.LineSegment:
                    if firstLine is None:
                        ...
                        if (
                            firstDirection == e.Curve.EndPoint - e.Curve.StartPoint
                            or firstDirection == e.Curve.StartPoint - e.Curve.EndPoint
                        ):
                        ...
                        for i in range(len(allEdges)):
                            try:
                                if type(allEdges[i].Curve) != Part.LineSegment:
                                    continue
                                if (
                                    allEdges[i].Curve.StartPoint
                                    == firstLine.Curve.StartPoint
                                    and allEdges[i].Curve.EndPoint
                                    == firstLine.Curve.EndPoint
                                ) or (
                                    allEdges[i].Curve.EndPoint
                                    == firstLine.Curve.StartPoint
                                    and allEdges[i].Curve.StartPoint
                                    == firstLine.Curve.EndPoint
                                ):
                                    firstLineIndex = i
                                elif (
                                    allEdges[i].Curve.StartPoint == e.Curve.StartPoint
                                    and allEdges[i].Curve.EndPoint == e.Curve.EndPoint
                                ) or (
                                    allEdges[i].Curve.EndPoint == e.Curve.StartPoint
                                    and allEdges[i].Curve.StartPoint == e.Curve.EndPoint
                                ):
                                    secondLineIndex = i
                                ...
                                continue
                        axis.References = [
                            (support, elementList[0]),
                            (support, "Edge" + str(firstLineIndex + 1)),
                            (support, "Edge" + str(secondLineIndex + 1)),
                        ]
                        axis.Offset = 1.0
                        axis.Offset2 = 1.0
                    ...
                    for i in range(len(allEdges)):
                        try:
                            ...
                            c = allEdges[i].Curve
                            if (
                                c.Center == e.Curve.Center
                                and c.Axis == e.Curve.Axis
                                and c.Radius == e.Curve.Radius
                            ):
                    ...
                    for i in range(len(allEdges)):
                        try:
                            ...
                            a = allEdges[i].Curve
                            if (
                                a.Center == e.Curve.Center
                                and a.Axis == e.Curve.Axis
                                and a.Radius == e.Curve.Radius
                                and a.FirstParameter == e.Curve.FirstParameter
                                and a.LastParameter == e.Curve.LastParameter
                            ):
                                axis.References = [(support, "Edge" + str(i + 1))]
                                self.feature.PositionType = "Coaxial"
                                break
        ...
        if not firstTime and len(refs) > 1:
            ...
            if (
                point.Point.distanceToLine(axisbase, axisdir) > 1e-10
            ):  # TODO: Precision::Confusion()
                found = True
        if not found:
            for p in face.OuterWire.Vertexes:
                if (
                    p.Point.distanceToLine(axisbase, axisdir) > 1e-10
                ):  # TODO: Precision::Confusion()
                    point = p
                    found = True
                    break
        if not found:
            point = face.OuterWire.Vertexes[
                0
            ]  # Better this than nothing... and it can't actually happen, can it?
    ...
    def setHoleDirection(self, feature):
        ...
        for v in sketch.Shape.Vertexes:
            ...
            if (
                v.Point.distanceToLine(axisbase, axisdir) < 1e-10
            ):  # TODO: use Precision::Confusion()
                if p1 is None:
                    p1 = v.Point
                else:
                    p2 = v.Point
    ...
    def createOrUpdateStandardSketch(self, fp, depth, radius):
        ...
        if fp.HoleGroove.Sketch.GeometryCount == 0:
            ...
            sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(10.0, 50.0, 0), self.App.Vector(10.0, -50.0, 0)
                )
            )  # Geo0: Rotation axis
            sketch.toggleConstruction(0)
            sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(10.0, -10.0, 0), self.App.Vector(10.0, -30.0, 0)
                )
            )  # Geo1: Vertical axis of hole
            sketch.addConstraint(
                Sketcher.Constraint("PointOnObject", 1, 1, 0)
            )  # Datum0
            sketch.addConstraint(
                Sketcher.Constraint("PointOnObject", 1, 2, 0)
            )  # Datum1
            sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(10.0, -10.0, 0), self.App.Vector(20.0, -10.0, 0)
                )
            )  # Geo2: Top of hole
            sketch.addConstraint(
            ...
            sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(20.0, -10.0, 0), self.App.Vector(20.0, -25.0, 0)
                )
            )  # Geo3: Vertical mantle of hole
            sketch.addConstraint(
                Sketcher.Constraint("Coincident", 2, 2, 3, 1)
            )  # temporary
            sketch.addConstraint(Sketcher.Constraint("Parallel", 3, 1))  # Datum4
            sketch.addConstraint(
                Sketcher.Constraint("Distance", 3, 2, 1, 10.0)
            )  # Datum5: Radius
            sketch.addConstraint(
                Sketcher.Constraint("Distance", 3, 2, 2, 15.0)
            )  # Datum6: Depth
            sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(10.0, -30.0, 0), self.App.Vector(20.0, -25.0, 0)
                )
            )  # Geo4: 118 degree tip angle
            sketch.addConstraint(
                Sketcher.Constraint("Coincident", 4, 1, 1, 2)
            )  # Datum7
            sketch.addConstraint(
                Sketcher.Constraint("Coincident", 4, 2, 3, 2)
            )  # Datum8
            ...
            sketch.addConstraint(
                Sketcher.Constraint("Angle", 4, 1, 1, 2, 118.0 / 2.0 * math.pi / 180.0)
            )  # Datum9
            # Locate at the intersection of the two external geometries
            sketch.addConstraint(
                Sketcher.Constraint("PointOnObject", 1, 1, -3)
            )  # Datum10
            sketch.addConstraint(
                Sketcher.Constraint("PointOnObject", 1, 1, -4)
            )  # Datum11
            sketch.addConstraint(
                Sketcher.Constraint("Angle", 0, 1, -3, 1, 0.0)
            )  # Datum12
        ...
        if self.oldCounterbore == True:
            ...
            if sketch.ExternalGeometry[1] != (support, elements[0]):
                ...
                sketch.addConstraint(
                    Sketcher.Constraint("Angle", 0, 1, -3, 1, angle)
                )  # Datum12
                sketch.addConstraint(
                    Sketcher.Constraint("Coincident", 2, 2, 3, 1)
                )  # Datum13
    ...
    def createOrUpdateCounterboreSketch(self, fp, depth, radius):
        ...
        if self.oldCounterbore == True:
            ...
            if sketch.ExternalGeometry[1] != (support, elements[0]):
                ...
                sketch.addConstraint(
                    Sketcher.Constraint("Distance", 2, cradius)
                )  # Datum13
                sketch.addConstraint(
                    Sketcher.Constraint("Coincident", 2, 2, 5, 1)
                )  # Datum14
                sketch.addConstraint(
                    Sketcher.Constraint("Distance", 3, 1, 2, cdepth)
                )  # Datum15
                ...
                sketch.addConstraint(
                    Sketcher.Constraint("Perpendicular", 6, -3)
                )  # Datum18
                sketch.addConstraint(
                    Sketcher.Constraint("Coincident", 6, 2, 3, 1)
                )  # Datum19
        else:
            ...
            sketch.addGeometry(
                Part.LineSegment(p2, self.App.Vector(p2.x, p2.y - 20.0, 0))
            )  # Geo5: Vertical mantle of counterbore
            sketch.addConstraint(
                Sketcher.Constraint("Coincident", 2, 2, 5, 1)
            )  # Datum14
            sketch.addConstraint(
            ...
            sketch.addGeometry(
                Part.LineSegment(self.App.Vector(p2.x, p2.y - 20.0, 0), p3)
            )  # Geo6: bottom of counterbore
            sketch.addConstraint(
                Sketcher.Constraint("Coincident", 5, 2, 6, 1)
            )  # Datum17
    ...
    def createOrUpdateCountersinkSketch(self, fp, depth, radius):
        ...
        if self.oldCountersink == True:
            ...
            if sketch.ExternalGeometry[1] != (support, elements[0]):
                ...
                sketch.addConstraint(
                    Sketcher.Constraint("Distance", 2, sradius)
                )  # Datum13
                sketch.addConstraint(
                    Sketcher.Constraint("Coincident", 2, 2, 5, 1)
                )  # Datum14
                sketch.addConstraint(
                    Sketcher.Constraint("Angle", 5, 2, 1, 2, sangle)
                )  # Datum15
                sketch.addConstraint(
                    Sketcher.Constraint("Coincident", 3, 1, 5, 2)
                )  # Datum16
        else:
