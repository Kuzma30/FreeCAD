...
class TestPrimitive(unittest.TestCase):
    ...
    def testPrimitiveCylinder(self):
        ...
        self.Doc.recompute()
        self.Cylinder001 = self.Doc.addObject(
            "PartDesign::SubtractiveCylinder", "Cylinder001"
        )
        ...
        self.assertAlmostEqual(
            self.Cylinder001.Shape.Volume, pi * 10 * (11 ** 2 - 10 ** 2)
        )
    ...
    def testPrimitiveSphere(self):
        ...
        self.Doc.recompute()
        self.Sphere001 = self.Doc.addObject(
            "PartDesign::SubtractiveSphere", "Sphere001"
        )
        self.Sphere001.Radius = 5
        self.Body.addObject(self.Sphere001)
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Sphere001.Shape.Volume, 4 / 3.0 * pi * (6 ** 3 - 5 ** 3)
        )
    ...
    def testPrimitiveCone(self):
        ...
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Cone001.Shape.Volume, 1 / 3.0 * pi * 10 * (4 ** 2 - 3 ** 2)
        )

    def testPrimitiveEllipsoid(self):
        self.Body = self.Doc.addObject("PartDesign::Body", "Body")
        self.Ellipsoid = self.Doc.addObject(
            "PartDesign::AdditiveEllipsoid", "Ellipsoid"
        )
        self.Ellipsoid.Radius1 = 2
        self.Ellipsoid.Radius2 = 4
        self.Body.addObject(self.Ellipsoid)
        self.Doc.recompute()
        self.Ellipsoid001 = self.Doc.addObject(
            "PartDesign::SubtractiveEllipsoid", "Ellipsoid001"
        )
        ...
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Ellipsoid001.Shape.Volume,
            4 / 3.0 * pi * (2 * 4 ** 2 - 1.5 * 3 ** 2),
            places=1,
        )
    ...
    def testPrimitiveTorus(self):
        ...
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Torus001.Shape.Volume, 2 * pi ** 2 * 10 * (4 ** 2 - 3 ** 2)
        )
    ...
    def testPrimitivePrism(self):
        ...
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Prism001.Shape.Volume, 3 * sqrt(3) / 2.0 * 10 * (4 ** 2 - 3 ** 2)
        )
    ...
    def testPrimitiveWedge(self):
        ...
        self.Doc.recompute()
        self.assertAlmostEqual(
            self.Wedge001.Shape.Volume, 1 / 2.0 * (10 * 10 - 9 * 8) * 10
        )
