...
def compute():
    ...
    # Addendum circle
    addendumCircle = FreeCAD.ActiveDocument.addObject(
        "Part::FeaturePython", "AddendumCircle"
    )
    ...
    for t in range(0, 60, 1):
        x = (
            db
            / 2
            * (math.cos(math.radians(t)) + math.radians(t) * math.sin(math.radians(t)))
        )
        y = (
            db
            / 2
            * (math.sin(math.radians(t)) - math.radians(t) * math.cos(math.radians(t)))
        )
    ...
    diff = (
        invPoint.y * 2
    )  # instead of making axial symmetry and calculating point distance.
