...
class PartDesignGuiTestCases(unittest.TestCase):
    ...
    def testRefuseToMoveSingleFeature(self):
        ...
        geoList = []
        geoList.append(
            Part.LineSegment(App.Vector(2.0, 8.0, 0), App.Vector(8.0, 8.0, 0))
        )
        geoList.append(
        ...
        self.assertEqual(
            len(self.BodySource.Group), 3, "Source body feature count is wrong"
        )
        self.assertEqual(
            len(self.BodyTarget.Group), 0, "Target body feature count is wrong"
        )
    ...
    def testMoveSingleFeature(self):
        ...
        geoList.append(
            Part.LineSegment(
                App.Vector(-10.000000, 10.000000, 0),
                App.Vector(10.000000, 10.000000, 0),
            )
        )
        geoList.append(
        ...
        self.assertFalse(
            self.Sketch.Support[0][0] in self.BodySource.Origin.OriginFeatures
        )
        self.assertTrue(
            self.Sketch.Support[0][0] in self.BodyTarget.Origin.OriginFeatures
        )
        self.assertEqual(
            len(self.BodySource.Group), 0, "Source body feature count is wrong"
        )
        self.assertEqual(
            len(self.BodyTarget.Group), 2, "Target body feature count is wrong"
        )
...
class PartDesignTransformed(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if (
            ...
            and App.ActiveDocument.Name == PartDesignTransformed
        ):
            App.closeDocument("PartDesignTransformed")
