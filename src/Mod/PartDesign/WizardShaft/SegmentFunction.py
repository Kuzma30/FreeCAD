...
class SegmentFunctionSegment:
    ...
    def clone(self):
        return SegmentFunctionSegment(
            self.start, self.variable, self.coefficient, self.exponent
        )
...
class IntervalFunction:
    ...
    def value(self, xval):
        for i in range(len(self.intervals)):
            if (
                xval >= self.intervals[i][0]
                and xval < self.intervals[i][0] + self.intervals[i][1]
            ):
...
class StressFunction:
    ...
    def evaluate(self, maxX, pointsX):
        ...
        xvals = set(
            [self.segfunc.segments[0].start + s * offset for s in range(pointsX)]
        )
        starts = set(
            [self.segfunc.segments[i].start for i in range(len(self.segfunc.segments))]
        )
        xvals = xvals.union(starts)  # Make sure we have a point on each segment start
        divs = set(
            [self.intfunc.intervals[i][0] for i in range(len(self.intfunc.intervals))]
        )
...
class TranslationFunction:
    ...
    def __init__(self, f, E, d, tangents, translations):
        ...
        self.tangfunc = (
            f.integrated()
        )  # The segment function for the tangent to the bending line
        ...
        A = np.zeros(
            shape=(2 * len(self.intfunc.intervals), 2 * len(self.intfunc.intervals))
        )
        ...
        for i in range(len(self.intfunc.intervals) - 1):
            ...
            b[row] = self.tangfunc.value(x_end) / (E * I_ip1) - self.tangfunc.value(
                x_end
            ) / (E * I_i)
            row += 1
            ...
            b[row] = self.transfunc.value(x_end) / (E * I_ip1) - self.transfunc.value(
                x_end
            ) / (E * I_i)
            row += 1
    ...
    def evaluate(self, maxX, pointsX):
        ...
        xvals = set(
            [self.transfunc.segments[0].start + s * offset for s in range(pointsX)]
        )
        starts = set(
            [
                self.transfunc.segments[i].start
                for i in range(len(self.transfunc.segments))
            ]
        )
        xvals = xvals.union(starts)  # Make sure we have a point on each segment start
        divs = set(
