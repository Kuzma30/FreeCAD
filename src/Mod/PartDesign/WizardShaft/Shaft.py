...
class Shaft:
            ...
            if self.segments[index].constraint is None:
                if constraintType == "Force":
                    ...
                    constraint = self.doc.addObject(
                        "Fem::ConstraintForce", "ShaftConstraintForce"
                    )
                    ...
                    constraint = self.doc.addObject(
                        "Fem::ConstraintFixed", "ShaftConstraintFixed"
                    )
                    if index == 0:
                    ...
                    constraint.References = [
                        (self.feature.feature, "Face%u" % (2 * (index + 1)))
                    ]
                    ...
                    constraint = self.doc.addObject(
                        "Fem::ConstraintGear", "ShaftConstraintGear"
                    )
                    constraint.References = [
                        (self.feature.feature, "Face%u" % (2 * (index + 1)))
                    ]
    ...
    def updateEdge(self, column, start):
        App.Console.PrintMessage(
            "Not implemented yet - waiting for robust references..."
        )
        return
        """
    ...
    def equilibrium(self):
        ...
        for i in range(len(self.segments)):
            ...
            if cType == "Fixed":
                ...
                location = (
                    self.getLengthTo(i) + self.segments[i].length / 2.0
                ) / 1000.0
                # The force itself
                for ax in range(3):
                    if abs(force[ax]) > 0.0:
                ...
                if abs(force[0]) > 0.0:
                    ...
                    coefficientsM[1][0] = (
                        coefficientsM[1][0] + momentz
                    )  # moment around z-axis
                    self.addTo(moments[1], location, momentz)
                    coefficientsM[2][0] = (
                        coefficientsM[2][0] - momenty
                    )  # moment around y-axis
                ...
                forceAngle1 = (
                    (constraint.ForceAngle + constraint.BeltAngle + 90.0)
                    / 180.0
                    * math.pi
                )
                forceAngle2 = (
        ...
        for ax in range(3):
            ...
            for i in range(len(self.segments)):
                ...
                bendingmoments[ax].addInterval(
                    location, length, areamoment / (od / 2.0)
                )
                torquemoments[ax].addInterval(
                    location, length, 2 * (areamoment / (od / 2.0))
                )
                location += length
