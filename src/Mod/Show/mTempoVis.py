...
S_INTERNAL = (
    3  # TV instance is being used by another TV instance as a redo data storage
)
...
class TempoVis(object):
    ...
    def modifyVPProperty(
        self, doc_obj_or_list, prop_names, new_value=JUST_SAVE, mild_restore=None
    ):
        ...
        if self.state == S_RESTORED:
            Wrn(
                "Attempting to use a TV that has been restored. There must be a problem with code."
            )
            return
    ...
    def hide(self, doc_obj_or_list, links_too=True, mild_restore=None):
        ...
        if links_too:
            self.modifyVPProperty(
                doc_obj_or_list, "LinkVisibility", False, mild_restore
            )
    ...
    def restore_all_dependent(self, doc_obj):
        ...
        self.restoreVPProperty(
            getAllDependent(doc_obj), ("Visibility", "LinkVisibility")
        )
