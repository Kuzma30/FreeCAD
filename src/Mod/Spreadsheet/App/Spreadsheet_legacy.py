...
class MathParser:
    ...
    def parseVariable(self):
        ...
        if value is None:
            raise ValueError("Unrecognized variable: '" + var + "'")
        return float(value)
    ...
    def __init__(self, obj=None):
        if obj:
            ...
            obj.addProperty(
                ...
                "Controllers",
                "Base",
                "Cell controllers of this object",
            )
...
class SpreadsheetController:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "FilterType",
            "Filter",
            "The type of filter to apply to the scene objects",
        )
        obj.addProperty(
            "App::PropertyString",
            "Filter",
            "Filter",
            "The filter to apply to the scene objects",
        )
        obj.addProperty(
            "App::PropertyEnumeration",
            "DataType",
            "Data",
            "The type of data to extract from the objects",
        )
        obj.addProperty(
            "App::PropertyString",
            "Data",
            "Data",
            "The data to extract from the objects",
        )
        obj.addProperty(
            "App::PropertyString",
            "BaseCell",
            "Base",
            "The starting cell of this controller",
        )
        obj.addProperty(
            "App::PropertyEnumeration",
            "Direction",
            "Base",
            "The cells direction of this controller",
        )
                ...
                for i in range(len(dataset)):
                    ...
                    if spreadsheet.Proxy.isKey(cell):
                        ...
                        try:
                            ...
                            if DEBUG:
                                print("setting cell ", cell, " to value ", value)
                        except:
                            print(
                                "Spreadsheet: Error retrieving property "
                                + obj.Data
                                + " from object "
                                + dataset[i].Name
                            )
...
class SpreadsheetPropertyController:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "TargetType",
            "Base",
            "The type of item to control",
        )
        obj.addProperty(
            "App::PropertyLink",
            "TargetObject",
            "Base",
            "The object that must be controlled",
        )
        obj.addProperty(
            "App::PropertyString",
            "TargetProperty",
            "Base",
            "The property or constraint of the target object to control",
        )
        obj.addProperty(
            "App::PropertyString",
            "Cell",
            "Base",
            "The cell that contains the value to apply to the property",
        )
        ...
        if obj.Cell and obj.TargetObject and obj.TargetProperty and obj.InList:
            ...
            if Draft.getType(sp) == "Spreadsheet":
                try:
                    ...
                    if DEBUG:
                        print("No value for cell ", obj.Cell, " in spreadsheet.")
                    return
                if obj.TargetType == "Property":
                    ...
                    if Draft.getType(obj.TargetObject) == "Sketch":
                        if obj.TargetProperty.isdigit():
                            ...
                            try:
                                obj.TargetObject.setDatum(
                                    obj.TargetProperty, float(value)
                                )
                                FreeCAD.ActiveDocument.recompute()
                                if DEBUG:
                                    print(
...
class SpreadsheetView(QtGui.QWidget):
    ...
    def __init__(self, spreadsheet=None):
        ...
        self.applyButton.setToolTip(
            translate("Spreadsheet", "Apply the changes to the current cell")
        )
        ...
        self.wipeButton.setToolTip(
            translate("Spreadsheet", "Deletes the contents of the current cell")
        )
        ...
        self.computeButton.setToolTip(
            translate("Spreadsheet", "Updates the values handled by controllers")
        )
        ...
        QtCore.QObject.connect(
            self.table, QtCore.SIGNAL("cellChanged(int,int)"), self.changeCell
        )
        QtCore.QObject.connect(
            self.table,
        ...
        QtCore.QObject.connect(
            self.lineEdit, QtCore.SIGNAL("returnPressed()"), self.getEditLine
        )
        QtCore.QObject.connect(
            self.applyButton, QtCore.SIGNAL("clicked()"), self.getEditLine
        )
        QtCore.QObject.connect(
            self.wipeButton, QtCore.SIGNAL("clicked()"), self.wipeCell
        )
        QtCore.QObject.connect(
            self.computeButton, QtCore.SIGNAL("clicked()"), self.recompute
        )
    ...
    def setEditLine(self, r, c, orr=None, orc=None):
        ...
        if self.spreadsheet:
            ...
            self.label.setText(
                str(translate("Spreadsheet", "Cell")) + " " + c.upper() + str(r) + " :"
            )
...
class _Command_Spreadsheet_Create:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Spreadsheet_Create", "Adds a spreadsheet object to the active document"
            ),
        }
    ...
    def Activated(self):
        ...
        FreeCAD.ActiveDocument.openTransaction(
            str(translate("Spreadsheet", "Create Spreadsheet"))
        )
...
class _Command_Spreadsheet_Controller:
    ...
    def GetResources(self):
        return {
            "Pixmap": "SpreadsheetController",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Spreadsheet_Controller", "Add controller"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Spreadsheet_Controller",
                "Adds a cell controller to a selected spreadsheet",
            ),
        }
    ...
    def Activated(self):
        ...
        if Draft.getType(FreeCADGui.Selection.getSelection()[0]) == "Spreadsheet":
            ...
            FreeCAD.ActiveDocument.openTransaction(
                str(translate("Spreadsheet", "Add controller"))
            )
            FreeCADGui.doCommand("import Spreadsheet")
            FreeCADGui.doCommand(
                "Spreadsheet.makeSpreadsheetController(FreeCAD.ActiveDocument."
                + n
                + ")"
            )
            FreeCAD.ActiveDocument.commitTransaction()
...
class _Command_Spreadsheet_PropertyController:
    ...
    def GetResources(self):
        return {
            "Pixmap": "SpreadsheetPropertyController",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Spreadsheet_PropertyController", "Add property controller"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "Spreadsheet_PropertyController",
                "Adds a property controller to a selected spreadsheet",
            ),
        }
    ...
    def Activated(self):
        ...
        if (len(sel) == 1) and Draft.getType(sel[0]) == "Spreadsheet":
            n = FreeCADGui.Selection.getSelection()[0].Name
            FreeCAD.ActiveDocument.openTransaction(
                str(translate("Spreadsheet", "Add property controller"))
            )
            FreeCADGui.doCommand("import Spreadsheet")
            FreeCADGui.doCommand(
                "Spreadsheet.makeSpreadsheetPropertyController(FreeCAD.ActiveDocument."
                + n
                + ")"
            )
            FreeCAD.ActiveDocument.commitTransaction()
            ...
            if (Draft.getType(sel[0]) == "Spreadsheet") and (
                Draft.getType(sel[1]) == "SpreadsheetPropertyController"
            ):
                s = sel[0].Name
                o = sel[1].Name
            elif (Draft.getType(sel[1]) == "Spreadsheet") and (
                Draft.getType(sel[0]) == "SpreadsheetPropertyController"
            ):
                s = sel[1].Name
                o = sel[0].Name
            else:
