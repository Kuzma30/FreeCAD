...
def getInfo(filename):
    ...
    tformat = FreeCAD.ParamGet(
        "User parameter:BaseApp/Preferences/Mod/Start"
    ).GetString("TimeFormat", "%m/%d/%Y %H:%M:%S")
    ...
    def getFreeDesktopThumbnail(filename):
        ...
        try:
            ...
            thumb = os.path.join(
                os.path.expanduser("~"), ".thumbnails", "normal", fhash + ".png"
            )
        else:
...
def handle():
    ...
    SECTION_RECENTFILES += (
        '<a href="LoadNew.py" title="' + TranslationTexts.T_CREATENEW + '">'
    )
    if FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool(
        ...
        SECTION_RECENTFILES += (
            '<img src="file:///'
            + os.path.join(resources_dir, "images/new_file_thumbnail.svg").replace(
                "\\", "/"
            )
            + '" alt="'
            + TranslationTexts.T_CREATENEW
    ...
    if FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool(
        ...
        if os.path.exists(examples_path):
            ...
            for basename in examples:
                ...
                SECTION_EXAMPLES += buildCard(
                    filename, method="LoadExample.py?filename="
                )
        SECTION_EXAMPLES += "</ul>"
    ...
    SECTION_CUSTOM = ""
    cfolders = FreeCAD.ParamGet(
        "User parameter:BaseApp/Preferences/Mod/Start"
    ).GetString("ShowCustomFolder", "")
    if cfolders:
        dn = 0
        for cfolder in cfolders.split(";;"):  # allow several paths separated by ;;
            ...
            if not os.path.exists(cfolder):
                ...
                SECTION_CUSTOM += (
                    "<h2>" + os.path.basename(os.path.normpath(cfolder)) + "</h2>"
                )
                SECTION_CUSTOM += "<ul>"
    ...
    HTML = HTML.replace(
        "IMAGE_SRC_USERHUB",
        "file:///"
        + os.path.join(resources_dir, "images/userhub.png").replace("\\", "/"),
    )
    HTML = HTML.replace(
        "IMAGE_SRC_POWERHUB",
        "file:///"
        + os.path.join(resources_dir, "images/poweruserhub.png").replace("\\", "/"),
    )
    HTML = HTML.replace(
        "IMAGE_SRC_DEVHUB",
        "file:///"
        + os.path.join(resources_dir, "images/developerhub.png").replace("\\", "/"),
    )
    HTML = HTML.replace(
        "IMAGE_SRC_MANUAL",
        "file:///"
        + os.path.join(resources_dir, "images/manual.png").replace("\\", "/"),
    )
    HTML = HTML.replace(
        "IMAGE_SRC_SETTINGS",
        "file:///"
        + os.path.join(resources_dir, "images/settings.png").replace("\\", "/"),
    )
    HTML = HTML.replace(
        "IMAGE_SRC_INSTALLED",
        "file:///"
        + os.path.join(resources_dir, "images/installed.png").replace("\\", "/"),
    )
    ...
    for wb in sorted(FreeCADGui.listWorkbenches().keys()):
        ...
        UL_WORKBENCHES += (
            '<a href="https://www.freecadweb.org/wiki/'
            + wn
            + '_Workbench">'
            + wn.replace("ReverseEngineering", "ReverseEng")
            + "</a>"
        )
        UL_WORKBENCHES += "</li>"
    UL_WORKBENCHES += "</ul>"
    ...
    SHADOW = "#888888"
    if (
        QtGui.QColor(BASECOLOR).valueF() < 0.5
    ):  # dark page - we need to make darker shadows
        SHADOW = "#000000"
    ...
    if FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Start").GetBool(
        ...
        HTML = HTML.replace(
            "width: 100%; /* thumbs display */", "width: 70%; /* thumbs display */"
        )
