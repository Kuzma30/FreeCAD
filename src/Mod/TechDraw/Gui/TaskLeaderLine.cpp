...
//    Base::Console().Message("TTL::setUiPrimary()\n");
    ...
    ui->pbTracker->setText(QCoreApplication::translate("Command", "Pick points"));
    if (m_haveMdi) {
...
//    Base::Console().Message("TTL::setUiEdit()\n");
    ...
    if (m_lineFeat != nullptr) {
        ...
        ui->pbTracker->setText(QCoreApplication::translate("Command", "Edit points"));
        if (m_haveMdi) {
            ui->pbTracker->setEnabled(true);
            ui->pbCancelEdit->setEnabled(true);
        } else {
...
//                            m_pbTrackerState);
    ...
    if (getCreateMode()) {
        ...
        ui->pbTracker->setText(QCoreApplication::translate("Command", "Save Points"));
        ui->pbTracker->setEnabled(true);
        ui->pbCancelEdit->setEnabled(true);
        ...
        if (!m_trackerPoints.empty()) {    //regular edit session
            ...
            if (qgLead == nullptr) {
                ...
                getMainWindow()->statusBar()->show();
                Gui::getMainWindow()->showMessage(msg,3000);
                ui->pbTracker->setText(QCoreApplication::translate("Command", "Save changes"));
                ui->pbTracker->setEnabled(true);
                ui->pbCancelEdit->setEnabled(true);
                m_pbTrackerState = TRACKERSAVE;
                enableTaskButtons(false);
