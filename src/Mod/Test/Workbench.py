...
class WorkbenchTestCase(unittest.TestCase):
    ...
    def testActivate(self):
        ...
        for i in wbs:
            try:
                ...
                self.assertTrue(
                    success, "Test on activating workbench {0} failed".format(i)
                )
    ...
    def testHandler(self):
        ...
        wbs = FreeCADGui.listWorkbenches()
        self.failUnless(
            "UnitWorkbench" in wbs, "Test on adding workbench handler failed"
        )
        ...
        wbs = FreeCADGui.listWorkbenches()
        self.failUnless(
            not "UnitWorkbench" in wbs, "Test on removing workbench handler failed"
        )
