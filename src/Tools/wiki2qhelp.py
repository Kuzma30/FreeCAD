...
OUTPUTPATH = (
    os.path.expanduser("~") + os.sep + ".FreeCAD"
)  # Where to store the qch file
...
def buildpdffiles(folder=TMPFOLDER, converter=PDFCONVERTER):
    ...
    for f in fileslist:
        if converter == "pisa":
            createpdf_pisa(f[:-5], folder)
        else:
            createpdf_htmldoc(f[:-5], folder)
...
def createpdf_pisa(pagename, folder=TMPFOLDER):
    ...
    if VERBOSE:
        print("Converting " + pagename + " to pdf...")
...
def joinpdf(folder=TMPFOLDER, startpage=INDEX, outputname="freecad.pdf"):
    ...
    for p in pages:
        if exists(p[:-5]):
            if VERBOSE:
                print("Appending", p)
            try:
                inputfile = PdfFileReader(open(folder + os.sep + p[:-5] + ".pdf", "rb"))
            except Exception:
    ...
    outputfile.close()
    if VERBOSE:
        print("Successfully created", OUTPUTPATH, os.sep, outputname)
...
def compile(qhpfile, outputname="freecad.qch"):
    ...
    if not os.system(QHELPCOMPILER + " " + qhpfile + " -o " + qchfile):
        if VERBOSE:
            print("Successfully created", qchfile)
        return 0
...
The help files for FreeCAD.
    ...
    if not os.system(QCOLLECTIOMGENERATOR + " " + qhcpfile + " -o " + qhcfile):
        if VERBOSE:
            print("Successfully created ", qhcfile)
        return 0
...
def cleanhtml(html):
    ...
    html = re.compile("(.*)<div[^>]+column-content+[^>]+>").sub(
        "", html
    )  # stripping before content
    html = re.compile("<div[^>]+column-one+[^>]+>.*").sub(
        "", html
    )  # stripping after content
    ...
    html = re.compile('<div id="jump-to-nav"[^>]*>.*?</div>').sub(
        "", html
    )  # removing nav div
    html = re.compile('<h3 id="siteSub"[^>]*>.*?</h3>').sub(
        "", html
    )  # removing print subtitle
    html = re.compile("Retrieved from").sub(
        "Online version:", html
    )  # changing online title
    html = re.compile('<div id="mw-normal-catlinks[^>]>.*?</div>').sub(
        "", html
    )  # removing catlinks
    html = re.compile('<div class="NavHead.*?</div>').sub(
        "", html
    )  # removing nav stuff
    html = re.compile('<div class="NavContent.*?</div>').sub(
        "", html
    )  # removing nav stuff
    html = re.compile('<div class="NavEnd.*?</div>').sub("", html)  # removing nav stuff
    if not GETTRANSLATIONS:
        html = re.compile('<div class="languages.*?</div>').sub(
            "", html
        )  # removing translations links
...
if __name__ == "__main__":
    # main(sys.argv[1:])
    print(
        "Warning! This script is obsolete. Use the scripts in the offlinedocs folder..."
    )
